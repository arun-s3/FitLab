            ...
            <section className={`relative mx-auto w-full max-w-6xl select-none`}>
                ...
                <div className='relative h-[260px] s-sm:h-[36rem] x-lg:h-[440px] overflow-hidden rounded-xl shadow-2xl bg-white'>
                    <div className='absolute inset-0'>
                        ...
                        <div className='pointer-events-none absolute inset-0 bg-gradient-to-r from-black/60 
                    ...
                    <div className='relative z-10 grid h-full grid-cols-1 x-lg:grid-cols-2'>
                        <div className='flex h-full items-center p-4 mob:p-6 sm:p-8 md:p-10'>
                            <AnimatePresence mode='popLayout' initial={false}>
                                ...
                                >
                                    <h2
                                        className='text-[22px] mob:text-[24px] xxs-sm:text-[26px] sm:text-[30px] md:text-[35px] 
                                            font-semibold tracking-tight line-clamp-2 break-words overflow-hidden text-white'>
                                        {slides[index].title}
                                    </h2>
                                    <p
                                        className='mt-2 mob:mt-3 text-[13px] mob:text-[14px] leading-relaxed line-clamp-3 
                                            break-words overflow-hidden text-white/85'
                                    >
                                        {slides[index].subtitle}
                                    </p>
                                    <button
                                        className='mt-3 mob:mt-4 inline-flex items-center rounded-md bg-primary px-3 
                                            mob:px-4 py-1.5 mob:py-2 text-[13px] mob:text-[14px] font-medium text-gray-900 
                                            shadow-sm transition-colors hover:bg-yellow-300 focus-visible:outline-none 
                                            focus-visible:ring-2 focus-visible:ring-primaryDark'
                                        type='button'
                                        onClick={() =>
                                            navigate(
                        ...
                        <div className='relative flex items-end justify-center md:justify-end pr-2 mob:pr-4 sm:pr-6 md:pr-10 pb-6 mob:pb-8'>
                            <motion.div className='relative flex t:hidden mob:hidden s-sm:flex'>
                                {deck.map((item) => {
                                    const pos = item.position
                                    const x = pos === 0 ? 0 : pos === 1 ? -70 : pos === 2 ? -35 : -5
                                    const y = -20
                                    ...
                                    return (
                                        ...
                                        >
                                            <div
                                                className='relative h-44 w-32 mob:h-52 mob:w-36 sm:h-60 sm:w-40 md:h-72 
                                                    md:w-48 lg:h-80 lg:w-56 rounded-[16px] overflow-hidden bg-transparent shadow-xl'>
                                                {!isActive && (
                                                    <motion.img
                            ...
                            <div
                                className='pointer-events-auto absolute bottom-[5px] x-lg:bottom-2 left-1/2 z-20 flex -translate-x-1/2 
