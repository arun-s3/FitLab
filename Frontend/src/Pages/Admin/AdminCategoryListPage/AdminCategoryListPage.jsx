...
export default function AdminCategoryListPage() {
    ...
    } = useSelector((state) => state.categoryStore)
                ...
                Object.keys(prevSubCategories).forEach((parentCat) => {
                    ...
                    if (blockedForParent.length) {
                        updatedSubCategories[parentCat] = updatedSubCategories[parentCat].map((cat) => {
                            const matchedBlockedCat = blockedForParent.find((blockedCat) => blockedCat.id === cat._id)

                            return matchedBlockedCat ? { ...cat, isBlocked: matchedBlockedCat.status } : cat
                        })
                    }
                })
                    ...
                    categories.map((category) => (
                        ...
                        <React.Fragment key={category._id}>
                            <tr id='category-content' className='border-b border-dashed border-[#84788A85]'>
                                <td
                                    colSpan='2'
                                    className='py-[20px] pl-[1.5rem]'
                                    style={
                                        isSubcategory
                                            ? { paddingLeft: `${parentLevelCount ? parentLevelCount * 3 : 2.5}rem` }
                                            : {}
                                    }
                                >
                                    <div className='flex items-center gap-[5px] relative'>
                                        {toggleTab.goTo !== "blocked" && toggleTab.goTo !== "active" &&
                                        category?.subCategory &&
                                        category?.subCategory.length ? (
                                            <i
                                                onClick={() => showSubcategories(category._id)}
                                                className='ml-[5px] absolute top-[8.3rem] left-[-34px] cursor-pointer'
                                                id='open-subcategory'
                                                data-label={
                                                    openSubcategories[category._id]?.["status"]
                                                        ? "Close Subcategories"
                                                        : "Show Subcategories"
                                                }
                                            >
                                        ...
                                        <div className='relative'>
                                            ...
                                            <div className='absolute left-[10px] top-[5px] flex gap-[25px] w-full'>
                                                <div className='flex items-center gap-[5px]'>
                                                    {category?.badge && (
                                                        <span
                                                            className=' border border-[#eae0f0] px-[10px] rounded-[7px] 
                                                                text-[10px] tracking-[0.3px] text-secondary'
                                                            style={!category.badge ? { display: "none" } : {}}>
                                                            {category?.badge}
                                                        </span>
                                                    )}
                                                    <span
                                                ...
                                                {(category?.productCount || category?.productCount === 0) && (
                                                    <span
                                                        className={`border border-[#eae0f0] px-[10px] rounded-[7px] flex 
                                                            items-center gap-[2px] font-[500] text-[10px] text-secondary
                                                            ${
                                                                category.isBlocked ||
                                                                (blockedCategoryList?.length &&
                                                                    blockedCategoryList?.find((cat) => cat.id == category._id)
                                                                        ?.status)
                                                                    ? "bg-red-500"
                                                                    : "bg-green-500"
                                                            }`}
                                ...
                                <td className='text-green-500'>
                                    {category?.relatedCategory
                                        ? category.relatedCategory.map((cat) => (
                                ...
                                <td className='text-muted text-[13px]'>
                                    <p>
                                        {category?.seasonalActivation?.startDate
                                            ? format(parseDate(category?.seasonalActivation?.startDate), "MMM dd, yy") +
                                              " (Starts)"
                                            : null}
                                    </p>
                                    <p className='mt-[5px]'>
                                        {category?.seasonalActivation?.endDate
                                            ? format(parseDate(category?.seasonalActivation?.endDate), "MMM dd, yy") +
                                              " (Ends)"
                                            : null}
                                    </p>
                                </td>
                                <td>
                                    <div className='w-[35px] flex flex-col gap-[2rem] text-secondary'>
                                        ...
                                        <span
                                            data-label={category.isBlocked ? "Unblock" : "Block"}
                                            className='w-[30px] p-[5px] border rounded-[20px] z-[2] flex items-center justify-center
                                                relative cursor-pointer admin-control'
                            ...
                            {openSubcategories[category._id]?.["status"]
                                ? subCategories &&
                                  ...
                                  tableBodyGenerator(
                                      ...
                                      true,
                                      openSubcategories[category._id]?.["parentLevelCount"] + 1,
                                  )
        ...
        <section id='AdminCategoryListPage'>
            ...
            <header>
                <AdminTitleSection
                    ...
                    subHeading='View, Edit and Organize all Categories seamlessly'
                />
            </header>
            ...
            <main>
                <div className='relative mt-[4.3rem]'>
                    <div
                        className='h-[40px] w-[150px] bg-white border border-b-[#e5e7eb] rounded-tl-[10px] cursor-pointer
                                 flex justify-center items-center absolute top-[-39px] tab'
                        onClick={() => showCategories("all")}
                        style={toggleTab.goTo == "all" ? { borderBottomColor: "transparent" } : {}}
                    >
                        <h4 className={toggleTab.goTo == "all" ? "opacity-[1]" : "opacity-[0.75]"}> All </h4>
                    </div>
                    <div
                        className='h-[40px] w-[150px] bg-white border border-b-[#e5e7eb] flex justify-center 
                            items-center absolute cursor-pointer top-[-39px] left-[150px] tab'
                        onClick={() => showCategories("active")}
                    ...
                    <div
                        className='h-[40px] w-[150px] bg-white border border-b-[#e5e7eb] flex justify-center items-center 
                            absolute cursor-pointer top-[-39px] left-[300px] tab'
                        onClick={() => showCategories("inactive")}
                    ...
                    <div
                        className='h-[40px] w-[150px] bg-white border border-b-[#e5e7eb] rounded-tr-[5px] flex 
                            justify-center items-center cursor-pointer absolute top-[-39px] left-[450px] tab'
                        onClick={() => showCategories("blocked")}
                    ...
                    <div className='border py-[1rem] px-[2rem] bg-white container'>
                        <input
                            ...
                            className={`h-[34px] lg:w-[47%] x-lg:w-[25rem] xl:w-[34rem] text-secondary border 
                                border-inputBorderLow rounded-[7px] placeholder:text-[11px] text-[13px] search-fitlab 
                                focus:shadow-lg focus:ring-secondary focus:border-secondary`}
                            onChange={(e) => searchHandler(e)}
                        />

                        <table cellSpacing={10} cellPadding={6} className='border-spacing-[24px] w-full mt-[2rem]'>
                            <thead>
                                <tr className='secondaryLight-box border border-[rgb(220, 230, 166)] font-[500] text-secondary 
                                    table-header'
                                >
                                    <td colSpan='2'>
