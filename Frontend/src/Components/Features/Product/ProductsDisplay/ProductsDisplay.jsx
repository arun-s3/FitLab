...
import WishlistOptionsModal from "../../Wishlist/WishlistModals/WishlistOptionsModal"
import RemoveWishlistItemModal from "../../Wishlist/WishlistModals/RemoveWishlistItemModal"
import ProductsTableView from "./ProductsTableView"
import RestockModal from "../RestockModal/RestockModal"
import ProductDiscountBadge from "../../../UI/Badges/DiscountBadges/ProductDiscountBadge"
import CategoryDiscountBadge from "../../../UI/Badges/DiscountBadges/CategoryDiscountBadge"
import PaginationV2 from "../../../UI/PaginationV2/PaginationV2"
import AnimatedStarGenerator from "../../../Tools/AnimatedStarGenerator/AnimatedStarGenerator"

import { toggleProductStatus, restockProduct } from "../../../../Slices/productSlice"
import {
    addProductToList,
...
import { SiteButtonSquare } from "../../../UI/SiteButtons/SiteButtons"
...
export default function ProductsDisplay({
    ...
    const { wishlist, wishlistProducts, listProductAdded, listProductRemoved, wishlistError } = useSelector(
        ...
        const getDiscountedCategories = async (categories) => {
            try {
                ...
                if (response.status === 200) {
                    const tranformedCategories = response.data.discountedCategories.map((cat) =>
        ...
        <>
                    ...
                    return (
                        ...
                        >
                            <h2 className='flex justify-between items-center'>
                                <span className='text-secondary text-[16px] font-[600] tracking-[0.4px]'>
                                    {" "}
                                    {list.name}{" "}
                                </span>
                                <span
                                    className={`px-[12px] py-[2px] text-[11px] font-[500] capitalize tracking-[0.3px] rounded-[3px] 
                                        ${wishlistPriorityDetails[index].bg} ${wishlistPriorityDetails[index].color} `}
                                >
                            ...
                            <h5 className='flex items-center gap-[1rem]'>
                                <span className='inline-flex items-center gap-[7px] text-muted'>
                                    ...
                                    <span className='text-[11px]'>
                                        {" "}
                                        Created On: {format(new Date(list.createdAt), "MMMM dd, yyyy")}{" "}
                                    </span>
                                </span>
                                <span className='inline-flex items-center gap-[7px] text-muted'>
            ...
            <motion.div
                ...
                className={`${
                    ...
                    ${products.length === 0 && "!flex !justify-center !items-center"}
                `}
                id='products-display'
            ...
            >
                {!showByTable && products && products.length > 0
                    ? products.map((product) => (
                        ...
                        >
                            <figure
                                className={`relative h-auto rounded-[10px] thumbnail cursor-pointer ${
                                    wishlistDisplay && !gridView
                                        ? "h-[300px]"
                                        : wishlistDisplay
                                        ? "h-[250px]"
                                        : gridView
                                        ? "h-[275px] xx-md:h-[200px] lg:h-[275px]"
                                        : "h-[275px]"
                                } ${productIsHovered === product._id && "shadow-lg"}`}
                            >
                                <img
                                    src={product?.thumbnail.url || product?.thumbnail || "/placeholder.svg"}
                                    alt={product.title}
                                    className={`rounded-[10px] 
                                        ${
                                            wishlistDisplay && !gridView
                                                ...
                                                ? "h-[250px]"
                                                : gridView
                                                ? "h-[275px] xx-md:h-[200px] xx-md:w-[200px] lg:h-[275px] lg:w-[275px]"
                                                : "h-[275px] w-[350px]"
                                        } object-cover
                                    `}
                                    onClick={() =>
                                ...
                                />
                                    ...
                                    >
                                        {admin ? (
                                            <div
                                                className='w-[35px] md:w-[92%] l-md:w-[35px] xx-lg:w-[92%] x-xl:w-[35px] flex flex-col
                                                       md:flex-row l-md:flex-col xx-lg:flex-row x-xl:flex-col gap-[1rem] 
                                                       md:justify-between l-md:justify-normal text-secondary
                                                       xx-lg:justify-between x-xl:justify-normal'
                                            >
                                                <span
                                                    data-label='Edit'
                            ...
                            <div
                                className={` ${
                                    gridView
                                        ...
                                        : `inline-flex flex-col gap-[10px] justify-between px-[1rem] py-[2rem] rounded-[10px] 
                                            ml-[1rem] product-infos w-full`
                                    } 
                                    ${wishlistDisplay && "mr-[1rem]"} 
                                    ${productIsHovered === product._id && "shadow-lg"} cursor-pointer
                                `}
                                onClick={() =>
                                    !admin &&
                                    navigate(
                            ...
                            >
                                <div>
                                    <p className='text-secondary flex items-center gap-[10px]'>
                                        ...
                                        <span className='text-secondary text-[14px] -mt-[11px]'>
                                            {" "}
                                    ...
                                    <p
                                        className={
                                            (gridView
                                                ...
                                                ? "text-[16px] font-[500]"
                                                : "text-[18px]") +
                                            " mt-[10px] capitalize font-[450] line-clamp-2 hover:text-secondary hover:underline"
                                        }
                                ...
                                {!gridView && (
                                    <p className={`${(admin && !gridView) || wishlistDisplay ? "text-[12px]" : "text-[14px]"} 
                                        text-wrap break-words line-clamp-3`}
                                        style={{ wordBreak: "break-word" }}
                                    >
                                  ...
                                  <div>
                                      {product.totalStock <= 5 ? (
                                        ...
                                        {(couponApplicableItems &&
                                            couponApplicableItems?.categories?.length > 0 &&
                                            couponApplicableItems.categories.some((category) =>
                                            product.category.includes(category.name),
                                          )) ||
                                            (couponApplicableItems &&
                                                couponApplicableItems?.products?.length > 0 &&
                                                couponApplicableItems.products.some(
                                                  (item) => item._id === product._id,
                                              ) && (
                                                    <p
                                                      className='mt-[10px] w-fit px-[7px] py-[3px] text-[13px] text-secondary 
                                                            font-medium italic bg-secondaryLight2 border rounded-[4px]'
                                                    >
                                        ...
                                        {wishlistDisplay && (
                                            <div>
                                                {product?.notes && (
                                                    <p className={`mt-[5px] flex items-center gap-[5px]`}>
                                                        <div className='flex items-center gap-[4px] text-muted'>
                                                            ...
                                                            <span className='text-[12px] font-[500] tracking-[0.5px]'>
                                                                {" "}
                                                                Your Note:{" "}
                                                            </span>
                                                        </div>
                                                        <span className='text-[12px] capitalize rounded-[3px]'>
                                                            {" "}
                                                            {product.notes}{" "}
                                                        </span>
                                                    </p>
                                                ...
                                                {product?.productPriority && (
                                                    <p
                                                        className={`mt-[5px] flex items-center gap-[5px] text-[13px] font-[500] 
                                                            tracking-[0.5px]`}
                                                    >
                                                        <span className='text-[12px] capitalize flex items-center gap-[4px]'>
                                                            ...
                                                            <div>
                                                                ...
                                                                <span
                                                                    className={`${
                                                                        wishlistPriorityDetails.find(
                                                                            (status) => status.value === product.productPriority
                                                                        ).color
                                                                    }`}
                                                                >
                                                                {
                                        ...
                                        {!gridView && (
                                            <div className='mt-[10px]'>
                                                {product.tags.map((tag) => (
                                                    <span
                                                        key={tag}
                                                        className={`px-[14px] py-[1px] 
                                                            ${admin ? "text-[12px]" : wishlistDisplay ? "text-[11px]" : "text-[13px]"} 
                                                            border-[1.5px]  border-[#eae0f0] rounded-[7px] text-secondary 
                                                            mr-[1rem]`
                                                        }
                                                    >
                                                      {tag}
                    ...
                    :   !showByTable && (
                            <h3 className='mt-[12rem] text-[13px] xs-sm2:text-[16px] xs-sm:text-[17px] text-muted capitalize 
                                tracking-[0.5px]'
                            >
            ...
            {showByTable && (
                <div>
                    <ProductsTableView
                        ...
                        restockProduct={openRestockModal}
                    />
                </div>
            )}
