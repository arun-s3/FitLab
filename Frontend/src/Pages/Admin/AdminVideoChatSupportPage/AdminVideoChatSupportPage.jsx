...
export default function AdminVideoChatSupportPage() {
    ...
    setPageBgUrl(
        `linear-gradient(to right,rgba(255,255,255,0.95),rgba(255,255,255,0.95)), url('/Images/admin-bg9.png')`,
    )
    ...
    const handleStartScheduledCall = (session) => {
        ...
        socket.emit("startScheduledCall", {
            adminId,
            adminSocketId: socket.id,
            userSocketId,
            currentScheduledSession: scheduledSessionNow,
        })
    }
    ...
    return (
        <section id='AdminVideoChatSupportPage' className='w-full -ml-[1.5rem] mb-[10rem]'>
            ...
            <header>
                <AdminTitleSection
                    heading='Video Chat Support'
                    subHeading='Real-time video assistance allows you to connect face-to-face with customers and resolve their queries.'
                />
            </header>
