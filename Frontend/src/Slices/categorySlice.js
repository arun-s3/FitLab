...
export const createCategory = createAsyncThunk("createCategory", async ({ formData }, thunkAPI) => {
    try {
        const response = await apiClient.post("/categories/add", formData, {
...
export const getSingleCategory = createAsyncThunk("getSingleCategory", async ({ id }, thunkAPI) => {
    try {
        ...
        return {
            populatedSubCategories: response.data.populatedSubCategories,
            parentLevelCount: response.data.parentLevelCount,
            id,
...
export const updateCategory = createAsyncThunk("updateCategory", async ({ formData, id }, thunkAPI) => {
    try {
        const response = await apiClient.post(`/categories/edit/${id}`, formData, {
            headers: { "Content-Type": "multipart/form-data" },
        })
        ...
        builder
            ...
            .addCase(getCategoryNames.fulfilled, (state, action) => {
                ...
                state.nestedSubcategoryNames = {
                    nestedSubcategoryNames: action.payload.nestedSubcategoryNames,
                    parentId: action.payload.id,
                }
            })
            ...
            .addCase(getNestedSubcategoryNames.fulfilled, (state, action) => {
                ...
                state.nestedSubcategoryNames = {
                    parentNameObj,
                    subCategoryNamesObj: nestedSubcategoryNames.filter((subCat) => subCat.parentLevelCount === 1),
                }
            })
            ...
            .addCase(updateCategory.fulfilled, (state, action) => {
                ...
                const updatingCategoryIndex = state.categories.findIndex(
