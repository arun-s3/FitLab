...
import { Tag, Percent, TrendingUp, TrendingDown, ArrowUp, ChevronUp, ChevronDown } from "lucide-react"
...
export default function OffersInsightsSection() {
    ...
    const { dashboardQuery, showOperationsAnalytics, setShowOperationsAnalytics } =
        useContext(OperationsAnalyticsContext)
    ...
    useEffect(() => {
        ...
        const orderedStats = offerStats.sort((a, b) => priorityOrder.indexOf(a.name) - priorityOrder.indexOf(b.name))
    ...
    return (
        ...
        >
            <h2 className='text-xl text-secondary font-bold flex items-center gap-[10px]'>
                <span className={`w-fit whitespace-nowrap ${!showOperationsAnalytics.offers && "text-muted"}`}>
                    {" "}
                    Offers Usage{" "}
                </span>
                <div
                    ...
                    onClick={() =>
                        togglerEnabled &&
                        setShowOperationsAnalytics((status) => ({ ...status, offers: !status.offers }))
                    }
                >
                    <hr
                    ...
                    >
                        ...
                        <motion.div
                            ...
                            className={`bg-white dark:bg-gray-800 p-6 rounded-lg shadow-sm border
                                ${dashboardQuery.trim() && !"Most Used Offers".toLowerCase().trim().includes(dashboardQuery.trim()) 
                                    ? "hidden" 
                                    : "inline-block"
                                }
                            `}
                            ...
                            <div className={`${status.mostUsedOffersDatas !== "error" ? "h-80" : "h-40"} relative`}>
                                {status.mostUsedOffersDatas !== "loading" ? (
                                    <ResponsiveContainer width='100%' height='100%'>
                                        ...
                                        ) : status.mostUsedOffersDatas !== "error" &&
                                          mostUsedOffersDatas.length === 0 ? (
                                            <p
                                                className='absolute -top-[10.5rem] sm:top-0 w-full h-full flex items-center 
                        ...
                        <div className='grid grid-cols-1 lg:grid-cols-2 gap-6'>
                            <motion.div
                                ...
                                className={`bg-white dark:bg-gray-800 p-6 rounded-lg shadow-sm border
                                    ${
                                        dashboardQuery.trim() &&
                                        !"Expired vs Active Offers Performance".toLowerCase().trim().includes(dashboardQuery.trim())
                                            ? "hidden"
                                            : "inline-block"
                                    }
                                `}
                                ...
                                <div className={`${status.monthlyOffersStats !== "error" ? "h-80" : "h-40"} relative`}>
                                    {status.monthlyOffersStats !== "loading" ? (
                                        <ResponsiveContainer width='100%' height='100%'>
                                            ...
                                            {status.monthlyOffersStats !== "error" &&
                                                monthlyOffersStats &&
                                                (monthlyOffersStats.length === 0 ||
                                                    monthlyOffersStats.every(
                            ...
                            <motion.div
                                {...fadeInUp}
                                className={`bg-white dark:bg-gray-800 p-6 rounded-lg shadow-sm border
                                    ${dashboardQuery.trim() && !"Offer Usage by User Type".toLowerCase().trim().includes(dashboardQuery.trim()) 
                                        ? "hidden" 
                                        : "inline-block"
                                    }`}
                                >
                                ...
                                <div
                                    className={`${status.offersByUserTypeStats !== "error" ? "h-80" : "h-40"} relative`}>
                                    {status.offersByUserTypeStats !== "loading" ? (

                                        <ResponsiveContainer width='100%' height='100%'>
                                            {status.offersByUserTypeStats !== "error" &&
                                            offersByUserTypeStats.length > 0 ? (
                                                <RadarChart outerRadius={90} data={offersByUserTypeStats}>
                                                    ...
                                                    />
                                                    {offersByUserTypeStats.length > 0 &&
                                                        Object.keys(offersByUserTypeStats[0])
                                                            .filter((key) => key !== "type")
                                                            .map((offerName, index) => (
                                                                <Radar
                                                                    key={offerName}
