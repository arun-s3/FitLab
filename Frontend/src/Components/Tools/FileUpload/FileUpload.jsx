...
export default function FileUpload({
    ...
    imagePreview,
    imageType,
    imageCropperPositionFromTop,
    imageCropperBgBlur,
    imageCropperContainerHt,
    editable = true,
    imageCropperControllerStyle,
    uploadBox,
    editingMode,
}) {
    ...
    useEffect(() => {
        ...
        if (error && error == "Please add files each of size below 3Mb!") {
            ...
            setImageMessage(
                "Please upload images smaller than 3MB. Images between 3MB and 5MB can be compressed or deleted.",
            )
            sonnerToast.warning(
                "Please upload images smaller than 3MB. Images between 3MB and 5MB can be compressed or deleted.",
            )
    ...
    const openImageEditor = (imageUrl, name, blob) => {
        ...
        windowRef.current = window.open(
            `${window.location.origin}/image-editor?name=${name}`,
            "",
            "width=1300,height=700,left=300,top=300,resizable=no",
        )
    ...
    return (
        <main className='w-full' id='fileupload'>
            <div
                className={`rounded-[5px] border border-dashed h-[17%] w-full flex justify-center
                ...
                style={
                    images.length > 0
                        ? { height: uploadBox && uploadBox?.afterUpload ? uploadBox.afterUpload : "120px" }
                        : { height: uploadBox && uploadBox?.beforeUpload ? uploadBox.beforeUpload : "240px" }
                }
            >
                <h3
                    className={`text-center align-middle my-auto text-[13px] ${checkDragging ? "text-[#5997f0]" : "text-secondary"}`}
                >
                    ...
                    <span>
                        <label
                            for='file'
                            className={`px-[8px] py-[2px] rounded-[5px] ml-[5px] text-black bg-primary 
                                    cursor-pointer ${checkDragging && "hidden"}`}
                        >
                            {" "}
                            Browse{" "}
                        </label>
                        <input
                            ...
                            id='file'
                            className='!hidden'
                            onChange={(e) => imageBrowseHandler(e)}
                            multiple
                        />
                    </span>
                </h3>
            ...
            <p className={`${error ? "text-red-500" : "text-muted"} text-[10px] h-[20px] mt-[5px]`}>
                {error
                    ? error
                    : `Upload images up to a maximum dimension of 400×400 pixels. 
            ...
            {imageCropperState && (
                ...
                <ImageCropper
                    ...
                    imageCropperDefaultIndex={imageCropperDefaultIndex}
                    imageCloseHandler={closeHandler}
                    imageCropperError={imageCropperError}
                    setImageCropperError={setImageCropperError}
                    positionFromTop={imageCropperPositionFromTop}
                    bgBlur={imageCropperBgBlur}
                    containerHeight={imageCropperContainerHt}
                            ...
                            >
                                ...
                                >
                                    ...
                                    {!imagePreview && editable && (
                                        <span
                                            className=' rounded-[4px] text-secondary absolute cursor-pointer
                                            bottom-[40px] text-[15px]'
                                            onClick={() => openImageEditor(image.url, image.name, image.blob)}>
                                            <RiImageEditLine />
                                        </span>
                                    )}
                                </span>
                                ...
                                {!image?.isCropped && (
                                    <span
                                        className='absolute bottom-[2px] left-[25%] px-[5px] py-[2px] rounded-[4px] bg-white text-[9px]
                                         font-[650] tracking-[0.6px] text-secondary cursor-pointer'
            ...
            <h5
                className='text-[12px] text-green-500 invisible h-[17px] mt-[5px] leading-[18px] tracking-[0.2px]'
                id='image-compress-message'
                style={displayCompressButton ? { color: "red", fontSize: "10px" } : {}}
            >
                {imageMessage}
                <span className='text-[18px] hidden' ref={imageMessageDisplay}>
                    {" "}
                    {imageMessage && " ...."}
                </span>
                {displayCompressButton && (
                    <button
                        className='px-[13px] py-[2px] rounded-[5px] ml-[1rem] text-black bg-primary cursor-pointer '
                        onClick={() => compressBigImages()}
                    >
                        Compress
                    </button>
                )}
            </h5>
            {needThumbnail && images.length > 0 ? (
                <div id='thumbnail-setter' className='mt-[2rem]'>
                    <h4 className='text-[13.5px] font-[500] text-black capitalize mb-[6px] ml-[2px]'>
                        {" "}
                    ...
                    <div className='flex gap-[15px] h-[200px]'>
                        ...
                        <div className='flex flex-col justify-between h-full arrows'>
                            <span onClick={() => arrowHandler("prev")}>
                                {" "}
                                <FaArrowUp />{" "}
                            </span>
                            <button
                                className='text-[12px] font-[500] tracking-[0.2px] py-[6px] w-[32px] h-[113px] 
                                ...
                                onClick={() =>
                                    openImageEditor(
                                        ...
                                        images[currentImageIndex].blob,
                                    )
                                }
                            >
                                Edit Image
                            </button>
                            <span onClick={() => arrowHandler("next")}>
                                {" "}
                                <FaArrowDown />{" "}
                            </span>
                        </div>
                ...
                <div className={`${imageType !== 'Profile pic' ? 'mt-8' : 'flex justify-center items-center'} 
                    ...
                    <div className={`${imagePreview?.size === "landscape" ? "w-full" : "w-[200px]"} `}>
                        ...
                        {imagePreview?.imageName && (
                            <span
                                className='absolute bottom-[52px] left-[10px] text-[10px] font-[550] text-secondary px-[10px] 
                                    rounded-[5px] tracking-[0.3px] capitalize category-name'>
                                {imagePreview.imageName.length > 20
                                    ? `${imagePreview.imageName[0].toUpperCase() + imagePreview.imageName.slice(3, 20)}...`
                                    : imagePreview.imageName[0].toUpperCase() + imagePreview.imageName.slice(1)}
                            </span>
                        )}
