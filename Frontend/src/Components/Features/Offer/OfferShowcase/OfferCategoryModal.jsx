...
export default function OfferCategoryModal({ categories, onClose }) {
    ...
    const getProductsCountUnderCategory = async () => {
        try {
            const response = await apiClient.get(`/categories/count/${categories[currentIndex]._id}`)
    ...
    return (
        ...
        >
            ...
            >
                    ...
                    <div className='p-6'>
                        <AnimatePresence mode='wait'>
                            <motion.div
                                ...
                                className='space-y-4'
                            >
                                <div className='bg-gradient-to-br from-slate-100 to-slate-200 rounded-lg h-40 flex 
                                ...
                                <div>
                                    ...
                                    <p className='text-slate-600  dark:text-slate-400 text-sm mb-3'>
                                        {capitalizeFirstLetter(categories[currentIndex]?.description) ||
                                            "No description available"}
                                    </p>

                                    {productCount ? (
                                        <div
                                            className='text-sm text-slate-700 dark:text-slate-300 bg-slate-100 
                                                tracking-[0.3px] px-3 py-2 rounded-lg inline-block'
                                        >
                ...
                {categories.length > 1 && (
                    <div className='flex items-center justify-between p-6 border-t border-slate-200 gap-4'>
                        <motion.button
                            whileHover={{ scale: 1.05 }}
                            whileTap={{ scale: 0.95 }}
                            onClick={handlePreviousCategory}
                            className='flex-1 flex items-center justify-center gap-2 bg-slate-200 hover:bg-slate-300 
                                text-slate-900 py-2 rounded-lg transition-colors font-medium'
                        >
