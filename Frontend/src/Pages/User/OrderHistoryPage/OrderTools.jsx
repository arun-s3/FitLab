        ...
        <div className='mb-[2rem] flex justify-between items-center' id='order-menu'>
            <div className='px-[8px] py-[6px] flex gap-[10px] bg-white rounded-[9px] border border-primary shadow-sm'>
                {orderTabs.map((tab) => (
                    <button
                        key={tab.name}
                        className={`px-[1rem] py-[8px] flex items-center gap-[5px] rounded-[7px] transition-all
                             duration-200 
                            ${
                                activeTab === tab.name
            ...
            <div className='flex justify-between items-center gap-[3rem]'>
                <div
                    className='h-[2.5rem] text-[14px] text-muted bg-white flex items-center gap-[10px]
                      justify-between border border-dropdownBorder rounded-[8px] shadow-sm cursor-pointer 
                      hover:bg-gray-50 transition-colors optionDropdownsort-dropdown'
                ...
                >
                    <span className='relative flex items-center border-r border-dropdownBorder py-[8px] pl-[10px] pr-[2px]'>
                        ...
                        {limit && openDropdowns.limitDropdown && (
                            <ul
                                className='absolute top-[44px] left-[3px] right-0 py-[10px] w-[101%] rounded-b-[4px] flex flex-col items-center
                ...
                <div
                    className='relative h-[2.5rem] px-[16px] py-[8px] text-[14px] text-muted bg-white flex items-center gap-[10px]
                        justify-between border border-dropdownBorder rounded-[8px] shadow-sm cursor-pointer hover:bg-gray-50 
                        transition-colors optionDropdown sort-dropdown'
                ...
                >
                    ...
                    {openDropdowns.sortDropdown && (
                        <ul
                            className='list-none  px-[10px] py-[1rem] absolute top-[44px] left-0 flex flex-col gap-[10px] justify-center 
                ...
                <div
                    className='relative h-[2.5rem] px-[16px] py-[8px] text-[13px] text-muted font-500 bg-white flex 
                        items-center gap-[10px] justify-between border border-dropdownBorder rounded-[8px] shadow-sm 
                        cursor-pointer hover:bg-gray-50 transition-colors optionDropdown'
                    id='duration-options'
                    onClick={() => toggleDropdown("durationDropdown")}
                    ref={dropdownRefs.durationDropdown}
                >
                    <span className='font-[470]'> {orderDuration} </span>
                    <CalendarArrowDown className='h-[15px] w-[15px]' /> 
