...
export default function CategoryDisplay({
    ...
    const closeNestedSubcategories = (id) => {
        if (subCategories[id]) {
            subCategories[id].forEach((subCategory) => {
                setOpenSubcategories((prevOpenSubcategories) => ({
                    ...prevOpenSubcategories,
                    [subCategory._id]: { ...openSubcategories[subCategory._id], status: false },
                }))
    ...
    const manualCheckboxHandler = (e, name) => {
        ...
        const manualCheckCategoryCurrentLength = Object.keys(manualCheckCategory).filter(
            (cat) => manualCheckCategory[cat] === true,
        ).length
        if (manualCheckCategoryCurrentLength > 2)
            setRelatedCategoryError("Can set only 3 categories as related category at the maximum!")
                                        ...
                                        >
                                            ...
                                            >
                                                {type == "checkboxType" ? (
                                                    <input
                                                        ...
                                                        onChange={(e) =>
                                                            checkboxHandler(e, category._id, category.subCategory)
                                                        }
                                                        checked={
                                            ...
                                            <div className='flex'>
                                                {type == "checkboxType" && category?.badge && (
                                                    <span className='border border-[#eae0f0] px-[10px] 
                                                        rounded-[7px] text-[10px] tracking-[0.3px] text-[#07bc0c]'
                                                    >
                                                        {category.badge}
                                                    </span>
                                                )}
