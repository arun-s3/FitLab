...
export default function AddressField({ infoArray, fieldType = "input", formData, onChange, onBlur, errors }) {
    ...
    const optionalFields = [
        {
            field: "nickName",
            placeholderMsg: "Optional: Keeping Nickname would give a personal touch, if it's a Gift Address",
        },
        ...
        <>
            {infoArray.map((info, index) =>
                fieldType !== "textarea" ? (
                    <div
                        key={info}
                        className={`${index === infoArray.length - 1 && !nonFullWidthFields.includes(info) ? "col-span-2" : null} 
                            relative`}
                    >
                        ...
                        <input
                            ...
                            placeholder={
                                optionalFields.some((fieldObj) => fieldObj.field === info)
