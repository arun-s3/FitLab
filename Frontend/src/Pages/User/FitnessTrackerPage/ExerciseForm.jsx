...
export default function ExerciseForm({ onAddOrUpdateExercise, editExerciseData = null, onExerciseUpdation }) {
    ...
    const fetchExercises = async (name) => {
        try {
            const response = await axios.get(`${exerciseAPiUrl}/exercises/filter`, {
                params: {
                    offset: 0,
                    limit: 20,
                    search: name,
    ...
    const updateExercise = async (exercise, id) => {
        try {
            ...
            if (!error.response) {
                ...
                sonnerToast.error(
                    error.response.data.message || "Error while updating your exercise. Please try later!",
                )
            } else {
                sonnerToast.error("Something went wrong! Please retry later.")
            }
                ...
                <div className='relative' ref={dropdownRefs.exerciseNamesDropdown}>
                    ...
                    <input
                        ...
                        className='w-full text-[14px] bg-gray-50 border border-gray-300 rounded-lg pl-10 pr-4 py-2 text-gray-900
                            placeholder-gray-500 focus:outline-none focus:border-purple-500 focus:ring-2 focus:ring-purple-200 
                            transition-all'
                    />
                    ...
                    {sets.map((set, index) => (
                        ...
                        >
                            ...
                            <div>
                                ...
                                />
                                <p className='text-gray-600 text-xs text-center mt-1 font-semibold'>
                                    RPE: {sets[index].rpe}/10
                                </p>
                            </div>
                            {index >= 1 && (
                                <button
            ...
            >
                {loading ? (
                    ...
                    <>
                        ...
                        <span>{!editExerciseData ? "Add Exercise" : "Update Exercise"}</span>
                    </>
                )}
            </motion.button>
