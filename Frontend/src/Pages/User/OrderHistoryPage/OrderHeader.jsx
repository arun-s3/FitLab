...
export default function OrderHeader({ order, onViewOrderDetails }) {
    ...
    const exportInvoice = async (orderId) => {
        try {
            ...
            if (!error.response) {
                ...
                sonnerToast.error(
                    error.response.data.message || "Error while exporting your invoice. Please try later!",
                )
            } else {
    ...
    return (
        <div
            className={`p-[1.5rem] grid grid-cols-4 gap-4 border-b border-dashed
                ${order.orderStatus === "cancelled" || order.orderStatus === "returning" 
                    ? "border-red-300" 
                    : "border-mutedDashedSeperation"}
            `}
        >
            ...
            {order.paymentDetails?.transactionId && (
                <div className='text-right space-y-1'>
                    ...
                    <div className='space-x-4 order-details'>
                        <Link className='hover:scale-105 transition duration-150' onClick={onViewOrderDetails}>
                        ...
                        <Link
                            ...
                            onClick={() => exportInvoice(order._id)}
                        >
