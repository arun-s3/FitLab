...
function validateCoachSessionState(state = {}) {
    ...
    validated.focus = CoachSessionStateSchema.focus.includes(state.focus) ? state.focus : DEFAULT_COACH_STATE.focus

    validated.lastRecommendation = CoachSessionStateSchema.lastRecommendation.includes(state.lastRecommendation)
