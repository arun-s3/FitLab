...
export default function ProductListingTools({
    ...
    const debouncedSearch = useRef(
        debounce((searchData) => {
            setQueryOptions((queryOptions) => ({ ...queryOptions, searchData: searchData }))
        }, 600),
    ).current
    ...
    return (
        <div
            className='w-[18rem] xxs-sm:w-[19rem] xs-sm2:w-auto flex flex-col justify-between lg:flex-row gap-[1rem] lg:gap-[2rem]'
        ...
        >
            <input
                ...
                className={`h-[34px] lg:w-[47%] x-lg:w-[25rem] xl:w-[34rem] text-secondary rounded-[7px] placeholder:text-[11px] text-[13px]
                  ${
                      wishlistDisplay
                          ? "border-2 border-dropdownBorder shadow-sm focus:ring-2 focus:ring-secondary focus:border-0 focus:shadow-lg"
            ...
            <div className='flex gap-[2rem] items-center justify-between lg:justify-normal'>
                {showFilter && (
                    <button
                        className='flex gap-[8px] items-center xx-md:hidden px-[10px] py-[5px] bg-whitesmoke hover:bg-purple-500  
                            text-secondary hover:text-white transition duration-150 border border-inputBorderLow 
                            hover:border-secondary rounded-[7px] cursor-pointer'
                        onClick={() => filterHandler()}
                    >
                        <VscSettings className='text-inherit' />
                        <h3 className='text-[13px] xs-sm:text-[14px] text-inherit font-[500] leading-[0.8px]'>
                            {" "}
                            Filter{" "}
                        </h3>
                    </button>
                ...
                {!wishlistDisplay && (
                    <div className='flex items-center gap-[10px]'>
                        ...
                        <div
                            className='flex items-center justify-center bg-secondaryLighter rounded-[4px] text-[12px] 
                                text-secondary py-[2px] xx-md:py-0 border border-inputBorderLow xx-md:border-primary 
                                px-[8px] relative z-[5] cursor-pointer'
                        ...
                        >
                            ...
                            {openDropdowns.limitDropdown && (
                                <ul
                                    className='absolute top-[22px] pt-[5px] w-[101%] rounded-[4px] flex flex-col items-center gap-[5px] 
                                border border-primary cursor-pointer'
                                >
                                    {limitValues.map((value, index) =>
                                        index !== limitValues.length - 1 ? (
                                            <li key={value} onClick={() => setLimit(value)}>
                                                {" "}
                                                {value}{" "}
                                            </li>
                                        ) : (
                                            <li key={value} className='pb-[5px]' onClick={() => setLimit(value)}>
                                                {" "}
                                                {value}{" "}
                                            </li>
                                        ),
                ...
                <div
                    className='flex items-center sort-by relative sort-dropdown cursor-pointer'
                ...
                >
                    ...
                    {openStickyDropdowns.sortDropdown && (
                        <ul
                            className='list-none cursor-pointer absolute top-[22px] flex flex-col gap-[10px] justify-center 
                                ...
                                >
                                    <span>
                                        <input
                                            ...
                                            checked={
                                                (Object.keys(sorts).find((key) => key === menuItem.value) &&
                                                    (menuItem?.order
                                                        ? sorts[menuItem.value] === menuItem.order
                                                        : true)) ||
                                                false
                                            }
                                        />
                ...
                {!admin && (
                    <div className='hidden x-lg:flex items-center gap-[5px] relative view-type'>
                        <span data-label='Grid View' onClick={() => setShowByGrid(true)}>
                            {" "}
                            <BsFillGrid3X3GapFill />{" "}
                        </span>
                        <span data-label='List View' onClick={() => setShowByGrid(false)}>
                            {" "}
                            <FaList />{" "}
                        </span>
                    </div>
