...
const calculateStreak = async (userId) => {
    ...
    const trackers = await FitnessTracker.find({ userId }).sort({ date: -1 }).lean()
...
const getWorkoutFrequencyStats = async (req, res, next) => {
    try {
        ...
        const weeklyWorkouts = weekDays.map((day, idx) => {
            const count = weeklyEntries.filter((entry) => new Date(entry.date).getDay() === (idx + 1) % 7).length
...
const getWeightInsights = async (req, res, next) => {
    try {
        ...
        const weeklyWeight = weekDays.map((day, index) => {
            const matching = weeklyData.find((w) => new Date(w.date).getDay() === index)
            return {
                day,
...
const getHealthScoreInsights = async (req, res, next) => {
    try {
        ...
        const monthlyMhs = monthlyData.map((item) => {
            const averageMhs = item.data.reduce((acc, h) => acc + calculateHealthScore(h), 0) / item.data.length
            return {
                day: `Week ${item._id.week}`,
