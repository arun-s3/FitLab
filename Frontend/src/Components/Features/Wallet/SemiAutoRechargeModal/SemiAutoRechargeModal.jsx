...
export default function SemiAutoRechargeModal({ isOpen, onClose, walletAmount, autoRechargeAmount, onRecharged }) {
            ...
            handler: async (response) => {
                try {
                    ...
                    if (verifiedData.status === 200) {
                        ...
                        const rechargeResponse = await apiClient.post(`/wallet/recharge/razorpay`, {
                            amount: data.amount,
                            razorpayPaymentId,
                        })
                    ...
                    if (!error.response) {
                        sonnerToast.error(
                            "Recharge verification failed due to network error. Please check your internet.",
                        )
    ...
    const skipRecharge = async () => {
        try {
            ...
            if (response.status === 200) {
                sonnerToast.success("Auto-recharge skipped!", {
                    description:
                        "It will retry when your balance falls below the threshold. Manage settings in your FitLab Wallet.",
                    duration: 6000,
                })
            }
        } catch (error) {
                        ...
                        >
                            <div className='relative px-6 sm:px-8 pt-6 sm:pt-8 pb-4 bg-gradient-to-r from-purple-500 to-purple-600'>
                                ...
                                >
                                    ...
                                    <div>
                                        <h2 className='text-lg sm:text-xl font-bold text-white'>Auto-Recharge</h2>
                                        <p className='text-[13px] text-purple-100'>Wallet semi-auto recharge</p>
                                    </div>
                                </motion.div>
                            </div>
                            ...
                            <div className='px-6 sm:px-8 py-6 sm:py-8'>
                                <div className='space-y-4 mb-4'>
                                    ...
                                    >
                                        ...
                                        <p className='text-[22px] font-bold text-gray-900'>{walletAmount}</p>
                                    </motion.div>
                                    ...
                                    >
                                        <div className='flex items-center justify-between mb-2'>
                                            <p className='text-sm text-gray-600 font-medium'>Recharge Amount</p>
                                            <Zap size={16} className='text-purple-500' />
                                        </div>
                                        <p className='text-[22px] font-bold text-gray-900'>{autoRechargeAmount}</p>
                                    </motion.div>
                                </div>
                                ...
                                >
                                    <p className='text-[13px] sm:text-[15px] text-gray-700 leading-relaxed text-center'>
                                        Your wallet is configured for semi-auto recharge. The system will proceed with
                                        recharging{" "}
                                        <span className='font-semibold text-gray-900'>{autoRechargeAmount}</span> in a
                                        few moments.
                                    </p>
                                </motion.div>
                                    ...
                                    <div className='relative w-20 h-20 flex items-center justify-center'>
                                        <svg className='absolute inset-0 transform -rotate-90' viewBox='0 0 100 100'>
                                            ...
                                            <motion.circle
                                                ...
                                                r='45'
                                                fill='none'
                                                stroke={isAutoClosing ? "#10b981" : "#9f2af0"}
                                                strokeWidth='3'
                                                strokeDasharray={`${((7 - timeRemaining) / 7) * 282.74} 282.74`}
                                ...
                                {isAutoClosing && (
                                    ...
                                    >
                                        ...
                                        <p className='text-sm font-medium'>Recharge initiating....</p>
                                    </motion.div>
                                )}
                            </div>
                        </div>
