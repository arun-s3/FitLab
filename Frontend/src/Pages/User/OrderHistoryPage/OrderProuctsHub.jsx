...
export default function OrderProuctsHub({
    order,
    product,
    onOpenOrderDetailsModal,
    onAddToCart,
    onCancelOrReturnProduct,
    onCancelReturnReq,
    onOpenChat,
}) {
        ...
        <div className='flex gap-[2rem]'>
            ...
            <div className='flex-1'>
                <h4 className='mb-[8px] flex items-center gap-[4rem]'>
                    ...
                    {order.products.some((item) => product.productStatus !== item.productStatus) && (
                        <span className='px-[5px] py-[3px] w-[10%] flex gap-[10px] items-center'>
                            <span
                                className={`w-[7px] h-[7px] ${findStyle(product.productStatus, "bg")} rounded-[5px]`}
                                style={{
                                    boxShadow: `0px 0px 6px 3px  ${findStyle(product.productStatus, "shadow")}`,
                                }}></span>
                            <span
                                className={`capitalize ${findStyle(product.productStatus, "textColor")} text-[13px] font-[500]`}>
                                {product.productStatus}
                            </span>
                        </span>
                    )}
                </h4>
                <p className='mb-[8px] text-[13px] text-gray-600 break-words whitespace-pre-wrap line-clamp-2'>
                    {product.subtitle}
                </p>
                {!product.productReturnStatus && (
                    <p
                        className='w-fit text-[13px] text-gray-500 tracking-[0.3px] flex items-center gap-[4px]
                    ...
                    >
                        {product.productStatus === "cancelled" || product.productStatus === "returning" ? (
                            <CircleOff className='w-[1rem] h-[1rem]' />
                        ) : (
                            <RefreshCcw className='w-[1rem] h-[1rem]' />
                        )}
                                  ...
                                  : product.productStatus === "delivered"
                                    ? Date.now() <= new Date(order.deliveryDate).getTime() + returnWindowTime
                                        ? "Return item: Eligible through " +
                                          format(
                                              new Date(order.deliveryDate).getTime() + returnWindowTime,
                                              "MMMM dd, yyyy",
                                          )
                                        : "Return date expired"
                                    : null}
                        {!["cancelled", "returning", "refunded", "delivered"].includes(product.productStatus) ? (
                            Date.now() > new Date(order.orderDate).getTime() + cancelWindowTime ? (
                                <span className='ml-[10px]'>
                                    {" "}
                                    Cancel Item: Not allowed as the 30-day period has expired.{" "}
                                </span>
                            ) : (
                                <span className='ml-[10px]'>
                                    {" "}
                                    Cancel Item: eligible within 30 days of delivery only.{" "}
                                </span>
                            )
                        ) : null}
                    </p>
                ...
                <div className='relative flex gap-[12px] mt-[1.5rem] item-buttons'>
                    <button
                        className='px-[16px] py-[4px] text-[14px] text-white bg-gradient-to-r from-[#B863F2]
                    ...
                    {!["cancelled", "returning", "refunded"].includes(product.productStatus) &&
                        !product.productReturnStatus && (
                            <motion.button
                                ...
                                onClick={(e) =>
                                    onCancelOrReturnProduct(e, product.productId, order._id, order.orderDate)
                                }
                            >
                    ...
                    {product.productId?._id && (
                        <button
                            className='border !border-primary hover:border-gray-300 transition-all duration-200'
                        ...
                        >
                            ...
                            {openDropdowns.showMoreDropdown && showMoreMenuOf === product?.productId?._id && (
                                <ul
                                    className='absolute !bottom-10 !right-[46%] bg-gray-50 border
                                        border-dropdownBorder py-4 rounded-[7px]'
                                >
                                    <li
                                        className='w-fit px-4 pb-[10px] flex items-center gap-[10px] 
                                    ...
                                    >
                                        ...
                                        <span className='text-[14px] text-inherit'>View Complete Order details</span>
                                    </li>
                                    <li
                                        className='w-fit px-4 pt-[10px] flex items-center gap-[10px] 
...

                {
                    product.productStatus === "returning" &&
                        ...
                        !order.orderReturnStatus ? (
                            ...
                            <RefundMessage refundReqAccepted={true} onOpenChat={onOpenChat} />
                          ...
                          !order.orderReturnStatus ? (
                            ...
                            <RefundMessage refundReqAccepted={false} onOpenChat={onOpenChat} />
