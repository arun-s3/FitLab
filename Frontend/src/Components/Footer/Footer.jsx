    ...
    <footer className="padding-main py-[2.3rem] text-white">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 lg:gap-14 items-start">
        <div className="space-y-4 w-[380px]">
          ...
          <nav className="ml-[28px] flex flex-wrap gap-4 items-center text-white" id="footer-socials" aria-label="Social media">
        ...
        {/* Links */}
        <nav className="mt-[24px] sm:ml-[28px] md:ml-[40px] x-md:ml-0 flex lg:ml-[4.5rem] xl:ml-0 gap-[4rem] lg:gap-[2rem] xl:gap-[4rem] items-center text-primary">
        ...
        <div className='sm:ml-[28px] md:ml-0'>
          ...
          <form className="flex flex-col sm:flex-row sm:justify-between items-stretch gap-3" onSubmit={(e) => e.preventDefault()}>
            <input
              type="email"
              placeholder="Your email"
              name="email"
              autoComplete="email"
              className="placeholder:text-[12px] py-[6px] pl-[14px] rounded-[7px] w-full md:w-auto lg:min-w-[12rem] xl:min-w-[17rem] bg-white/90 text-black"
            />
