...
export default function TransactionDetailsSection({transactions}){
    ...
    const getTransactionTypeStyle = (type)=> {
        switch(type){
            ...
            case 'request_sent':
                return {color: 'text-[#f1c40f]', bg: 'text-[#d7f148]', border: 'border-[#f1c40f]', icon: Pause, symbol: '#'}
            case 'request_received':
                return {color: 'text-[#f1c40f]', bg: 'text-[#d7f148]', border: 'border-[#d7f148]', icon: Pause, symbol: '#'}
        }
    }
        ...
        <section className="bg-white shadow-sm rounded-lg">
            <div className="p-6">
              <div className="flex justify-between items-center mb-4">
                ...
                {
                transactions.length > 0 ?
                <table cellSpacing={10} cellPadding={10} className="w-full border-separate border-spacing-y-4">
                  {
                    transactions.map(transaction=> (
                      (
                        ()=> {
                            const transactionColor = getTransactionTypeStyle(transaction.type).color
                            const transactionBg = getTransactionTypeStyle(transaction.type).bg
                            const transactionBorder = getTransactionTypeStyle(transaction.type).border
                            const TransactionIcon = getTransactionTypeStyle(transaction.type).icon
