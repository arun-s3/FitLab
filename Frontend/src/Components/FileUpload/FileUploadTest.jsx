...
export default function FileUpload({images, setImages, imageLimit, needThumbnail, thumbnail, setThumbnail, thumbnailIndexOnEditProduct, categoryImgPreview}){
    ...
    const [imageCropperState, setImageCropperState] = useState(true)
    ...
    const handleCropComplete = (croppedImg, image, index) => {
        setCroppedImages((croppedImages) => {
            return [...croppedImages, { ...image, url: croppedImg }];
        })
        if (index === images.length - 1) setImageCropperState(false);
    }
    ...
    return(
        <main className='w-full h-screen' id='fileupload'>
            ...
            {   imageCropperState &&
                <ImageCropper images={images} onCropComplete={handleCropComplete} setImageCropperState={setImageCropperState}/>
            }
