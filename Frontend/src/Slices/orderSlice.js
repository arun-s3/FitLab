...
export const cancelOrderProduct = createAsyncThunk(
    "order/cancelOrderProduct",
    async ({ orderId, productId, productCancelReason }, thunkAPI) => {
        try {
...
export const deleteProductFromOrderHistory = createAsyncThunk(
    "order/deleteProductFromOrderHistory",
...
export const changeProductStatus = createAsyncThunk(
    "order/changeProductStatus",
    async ({ orderId, productId, newProductStatus }, thunkAPI) => {
        try {
            const response = await apiClient.patch(`/order/status/${orderId}/products/${productId}`, {
                newProductStatus,
            })
...
export const initiateReturn = createAsyncThunk(
    ...
    async ({ orderId, productId, returnType, returnReason, formData }, thunkAPI) => {
        try {
            const response = await apiClient.post("/order/return", formData, {
...
export const handleReturnDecision = createAsyncThunk(
    "order/handleReturnDecision",
    async ({ returnDetails }, thunkAPI) => {
        try {
                        ...
                        order.products = order.products.map((product) => {
                            if (product.productId._id === action.payload.productId) {
                                ...
                                if (requiredStatus === "delivered") {
                                    const otherDeliveredProducts = order.products.find(
                ...
                if (state.orders && state.orders.length > 0) {
                    ...
                    if (order) {
                        if (action.payload.refundType === "product") {
                            ...
                            action.payload.refundType === "order" ||
                            action.payload?.fitlabAutoInitiated?.status
                        ) {
                            order.orderStatus = "refunded"
