...
const getExerciseVideos = async (req, res, next) => {
    try {
        ...
        if (videoResults.length > 0) {
            await Fitness.updateOne({ name }, { $set: { videos: videoResults } }, { upsert: true })
        }
...
const updateExerciseTemplate = async (req, res, next) => {
    try {
        ...
        if (equipment !== undefined) template.equipment = equipment.trim() || null
...
const updateWorkoutInfo = async (req, res, next) => {
    try {
        ...
        exercise.totalVolume = exercise.sets.reduce((acc, s) => acc + (s.weight || 0) * (s.reps || 0), 0)
...
const updateCaloriesForExercise = async (req, res, next) => {
    try {
        ...
        return res
            .status(200)
            .json({ success: true, message: "Calories updated successfully!", totalCalories: workout.totalCalories })
    } catch (error) {
        console.error(error)
        next(error)
    }
...
const getUserExerciseLibrary = async (req, res, next) => {
    try {
        ...
        const exercises = await ExerciseTemplate.find({ userId }).sort(sortQuery).skip(skip).limit(limit)
        return res.status(200).json({
            success: true,
            page,
            limit,
...
const getWorkoutHistory = async (req, res, next) => {
    try {
        ...
        const todayWorkouts = await FitnessTracker.find({
            ...
            .sort({ updatedAt: -1 })
            .lean()
        const olderWorkouts = await FitnessTracker.find({ userId, date: { $lt: todayStart } })
            .sort({ updatedAt: -1 })
            .skip(skip)
            .limit(limit)
            .lean()
    ...
    try {
        ...
        const result = await FitnessTracker.aggregate([
            ...
            { $unwind: "$exercises" },
            {
                $addFields: {
                    activityTime: {
                        $ifNull: ["$exercises.exerciseCompletedAt", "$updatedAt"],
                    },
                },
            },
            {
...
const addOrUpdateDailyHealthProfile = async (req, res, next) => {
    try {
        ...
        if (!age || !height || !weight) {
            return next(errorHandler(400, "Age, height and weight are required fields"))
        }
...
const getLatestHealthProfile = async (req, res, next) => {
    try {
        const userId = req.user._id
        const profiles = await HealthProfile.find({ userId }).sort({ date: -1 }).limit(2).lean()
        if (!profiles.length) {
