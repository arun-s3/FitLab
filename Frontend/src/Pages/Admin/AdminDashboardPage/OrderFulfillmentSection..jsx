    ...
    return (
        ...
        >
            <h2 className='text-xl text-secondary font-bold flex items-center gap-[10px]'>
                <span className={`w-fit whitespace-nowrap ${!showBusinessAnalytics.orders && "text-muted"}`}>
                    {" "}
                    Orders & Fulfillment{" "}
                </span>
                <div
                    ...
                    onClick={() =>
                        togglerEnabled && setShowBusinessAnalytics((status) => ({ ...status, orders: !status.orders }))
                    }
                >
                    <hr
            ...
            <AnimatePresence>
                                ...
                                <div
                                    ...
                                    {status.mostPurchasedProducts !== "loading" ? (
                                        <ResponsiveContainer width='100%' height='100%'>
                                            ...
                                            {topProductDatas &&
                                                status.mostPurchasedProducts !== "error" &&
                                                (topProductDatas.length === 0 ||
                                                    topProductDatas.every((data) => data.orders === 0)) && (
                                                    <p
                ...
                ${dashboardQuery.trim() && !"Order Status Distribution".toLowerCase().trim().includes(dashboardQuery.trim()) ? "hidden" : "inline-block"}`}>
                                ...
                                <div
                                    className={`${status.orderStatusDistribution !== "error" ? "h-64" : "h-40"} relative`}>
