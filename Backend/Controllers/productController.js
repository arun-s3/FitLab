        ...
        if (!product.variantOf) {
            ...
            if (mainProduct) {
                ...
                const variantValues = variantKeyMain
                    ? [mainProduct[variantKeyMain], ...mainProduct.variants.map((v) => v[variantKeyMain])].filter(
                          Boolean,
                      )
                    : []
...
const getAllProducts = async (req, res, next) => {
    try {
        ...
        const hasRatingFilter = typeof selectedRating === "number" && selectedRating > 0

        if (hasRatingFilter) {
            filteredProducts = filteredProducts.filter(
                (product) => typeof product.averageRating === "number" && product.averageRating >= selectedRating,
            )
        }
        ...
        if (maxStock !== undefined && maxStock !== null && maxStock !== "") {
            ...
            if (!isNaN(maxStock)) {
                filteredProducts = filteredProducts.filter(
                    (product) => typeof product.totalStock === "number" && product.totalStock <= maxStock,
                )
            }
        }
...
const getSimilarProducts = async (req, res, next) => {
    try {
        ...
        const similarMainProducts = await Product.find({
            ...
            variantOf: { $in: [null, undefined] },
            _id: { $nin: productIds },
            $or: [{ category: { $in: Array.from(allCategories) } }, { tags: { $in: Array.from(allTags) } }],
        })
            .limit(12)
            .populate({
                path: "variants",
...
const searchProduct = async (req, res, next) => {
    try {
        ...
        const filter = {
            $or: [{ title: { $regex: find, $options: "i" } }, { subtitle: { $regex: find, $options: "i" } }],
            isBlocked: false,
        }
        ...
        const formattedProducts = products.map((product) => {
            ...
            const priceLabel = minPrice !== maxPrice ? `₹${minPrice} - ₹${maxPrice}` : `₹${minPrice}`
        ...
        const allPrices = formattedProducts.flatMap((p) =>
            typeof p.price === "string" ? p.price.replace(/[₹\s]/g, "").split("-").map(Number) : [],
        )
    ...
    try {
        ...
        const formattedProducts = products.map((product) => {
            ...
            return {
                ...
                Weights: variantType === "weight" ? formatArray(product.weights) : "",
                Colors: variantType === "color" ? formatArray(product.colors) : "",
                Sizes: variantType === "size" ? formatArray(product.sizes) : "",
                "Motor Powers": variantType === "motorPower" ? formatArray(product.motorPowers) : "",
                Price: priceLabel,
...
const exportProductsAsPdf = async (req, res, next) => {
    try {
        ...
        products.forEach((product, index) => {
            ...
            const rowData = [
                product.title
                    ? String(product.title).length > 25
                        ? String(product.title).substring(0, 25) + "..."
                        : String(product.title)
                    : "N/A",
                product.category
                    ? String(product.category).length > 32
                        ? String(product.category).substring(0, 32) + "..."
                        : String(product.category)
                    : "N/A",
                product[`${product.variantType}s`]
                    ? String(product[`${product.variantType}s`]).length > 32
                        ? String(product[`${product.variantType}s`]).substring(0, 32) +
                          "..." +
                          variantSymbol[product.variantType]
                        : String(product[`${product.variantType}s`]) + variantSymbol[product.variantType]
                    : "N/A",
...
module.exports = {
    ...
    getPopularProducts,
    getSimilarProducts,
    getEquipmentByMuscleOrExercise,
    searchProduct,
    updateProduct,
    restockProduct,
    toggleProductStatus,
