...
const PaymentSummary = forwardRef(
    (
        { heading, absoluteTotal, absoluteTotalWithTaxes, deliveryCharge, couponDiscount, gst, setIsRemoveModalOpen },
        ref,
    ) => {
        ...
        return (
            <motion.div
                className='h-fit  bg-whitesmoke rounded-[8px] p-[1.5rem] border border-dropdownBorder shadow-md'
                id='order-summary'
            ...
            >
                ...
                <motion.div className='space-y-[1rem]' variants={containerVariants}>
                    <motion.div className='flex justify-between' variants={itemVariants}>
                        <span className='order-title'> Subtotal </span>
                        <span className='order-value tracking-[0.5px]'>
                            {" "}
                            ₹{absoluteTotal.toFixed(2).toLocaleString()}{" "}
                        </span>
                    </motion.div>
                    <motion.div className='flex justify-between' variants={itemVariants}>
                    ...
                    <motion.div className='flex justify-between' variants={itemVariants}>
                        <span className='order-title flex flex-col'>
                            ...
                            <span className='text-[10px] text-muted'>
                                {" "}
                                (12% for supplements and 18% for others are added to each item){" "}
                            </span>
                        </span>
                    ...
                    {couponDiscount && couponDiscount > 0 ? (
                        <motion.div variants={itemVariants}>
                            <div className='relative flex justify-between !mt-[2rem]'>
                                ...
                                <X
                                    className='absolute top-[5px] right-[-19px] w-[15px] h-[15px] text-red-500 cursor-pointer'
                ...
                <SiteButtonSquare
                    tailwindClasses='mt-[1rem] w-full hover:bg-primaryDark transition-colors'
