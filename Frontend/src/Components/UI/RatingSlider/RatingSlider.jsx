        ...
        >
                        ...
                        {[4, 3, 2, 1].map((rating) => (
                            <label key={rating} className='flex items-center space-x-2 cursor-pointer'>
                                <input
                                    ...
                                    className={`w-4 h-4 ${!enableRadioRate ? "text-muted" : "text-purple-600"} 
                                        border-gray-300 focus:ring-0 ${!enableRadioRate && "cursor-not-allowed"}`
                                    }
            ...
            <div className={`mb-8 relative w-full ${indentSlider ? `ml-[${indentSlider}]` : null}`}>
                <input
                    ...
                    className={`w-full h-[4px] appearance-none ${!enableSliderRate ? "bg-muted" : " bg-[#AFD0FF]"} 
                        rounded-[2px] cursor-pointer relative z-10 
                        ${!enableSliderRate ? "[&::-webkit-slider-thumb]:bg-muted" : "[&::-webkit-slider-thumb]:bg-secondary"} 
                        [&::-webkit-slider-thumb]:transition-transform [&::-webkit-slider-thumb]:hover:scale-110 
                        ${!enableSliderRate ? "[&::-moz-range-thumb]:bg-muted" : "[&::-moz-range-thumb]:bg-secondary"} 
                        [&::-moz-range-thumb]:border-0 [&::-moz-range-thumb]:transition-transform 
                        [&::-moz-range-thumb]:hover:scale-110 ${!enableSliderRate ? "cursor-not-allowed" : "cursor-grabbing"}`
                    }
                ...
                <div
                    className={`absolute left-0 top-1/2 -translate-y-1/2 w-4 h-4 rounded-full transition-all duration-200 
                        ${isDragging ? "ring-4 ring-secondary/20" : ""}`
                    }
                    style={{ left: `calc(${(sliderRate / 5) * 100}% - 8px)` }}
                />
