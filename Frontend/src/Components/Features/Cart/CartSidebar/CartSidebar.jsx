...
import ProductRemovalModal from "../../Product/ProductRemovalModal/ProductRemovalModal"

import { SiteSecondaryFillImpButton } from "../../../UI/SiteButtons/SiteButtons"
...
export default function CartSidebar({ isOpen, onClose, retractedView }) {
    ...
    const { cart, productAdded, productRemoved, error, couponMessage } = useSelector((state) => state.cart)
    ...
    return (
        <>
            ...
            <div
                id='ProductDetailSidebar'
                className={`fixed ${
                    scaleDownSidebar ? "top-[55%] h-auto border border-dropdownBorder" : "top-0 h-full"
                    } top-0 right-0 
                    ${retractedView || scaleDownSidebar 
                        ? "w-[140px] xxs-sm:w-[160px]" : "w-full max-w-[280px] xs-sm:max-w-[300px] lg:w-[400px]"
                    } bg-white shadow-2xl rounded-l-2xl transform transition-transform duration-300 ease-in-out
                ...
                <div className={`flex flex-col h-full ${scaleDownSidebar && "w-[130px] xxs-sm:w-[150px]"} `}>
                    <div className={`flex justify-between items-center p-3 xxs-sm:p-4 ${!scaleDownSidebar && "border-b"} `}>
                        ...
                        <div className='flex gap-[4px] xxs-sm:gap-[5px] items-center'>
                            {scaleDownSidebar && (
                                <ShoppingCart className='h-[17px] w-[17px] xxs-sm:h-[19px] xxs-sm:w-[19px] text-muted' />
                            )}
                            <h2 className={` ${
                                scaleDownSidebar 
                                    ? "text-[11px] xxs-sm:text-[12px]" 
                                    : retractedView 
                                    ? "text-[13px] xxs-sm:text-[14px]" 
                                    : "text-lg xxs-sm:text-xl"
                        ...
                        <i className='text-gray-500 hover:text-gray-700'>
                            ...
                            {scaleDownSidebar ? (
                                <SquareArrowOutUpLeft
                                    className='w-[16px] h-[16px] xxs-sm:w-[18px] xxs-sm:h-[18px] text-secondary cursor-pointer'
                                    onClick={() => setScaleDownSidebar(false)}
                                />
                            ) : (
                                <X
                                    size={20}
                                    className='xxs-sm:size-[24px] text-secondary cursor-pointer'
                                    onClick={onClose}
                                />
                            )}
                        </i>
                    ...
                    {!scaleDownSidebar && (
                        <div
                            className={`flex-1 relative ${packedupCart?.products?.length > 4 ? "overflow-y-scroll" : "overflow-y-auto"} 
                                p-3 xxs-sm:p-4 space-y-3 xxs-sm:space-y-4`}
                        >
                            {(packedupCart && Object.keys(packedupCart).length === 0) ||
                            (packedupCart?.products && packedupCart.products.length === 0) ? (
                                <p
                                    className={`h-full w-full text-muted capitalize flex items-center justify-center 
                                        text-[13px] xxs-sm:text-[14px] 
                                        ${retractedView && "text-[13px] xxs-sm:text-[14px]"}`}
                                >
                                    Your cart is empty
                                </p>
                            ) : (
                                packedupCart?.products.map((product) => (
                                    <div
                                        ...
                                        id='cart-product'
                                        className={`flex ${retractedView && "flex-col gap-[8px] xxs-sm:gap-[10px]"} items-center
                                    ...
                                    >
                                        {retractedView ? (
                                            <div className='flex gap-[4px] xxs-sm:gap-[5px]'>
                                                <img
                                                    ...
                                                    alt={product.title}
                                                    className='ml-[8px] xxs-sm:ml-[10px] w-[45px] h-[45px] xxs-sm:w-[50px] 
                                                        xxs-sm:h-[50px] rounded-lg object-cover'
                                                />
                                                <button
                                                    className='self-start text-red-500 hover:text-red-700'
                                                    onClick={() =>
                                                        removeProductFromCart(product.productId._id, product.title)
                                                    }
                                                >
                                            ...
                                            <img
                                                src={product.thumbnail}
                                                alt={product.title}
                                                className='w-[45px] h-[45px] xxs-sm:w-[50px] xxs-sm:h-[50px] rounded-lg object-cover'
                                            />
                                        )}
                                                ...
                                                <p className='text-[10px] xxs-sm:text-[11px] text-secondary truncate'>
                                                    <span
                                                        className={` ${
                                                            ...
                                                            `mr-[3px] xxs-sm:mr-[4px] line-through decoration-[1.6px] 
                                                                decoration-red-500`
                                                        }`}
                                                    >
                                                        &#8377;{product.price}
                                        ...
                                        <div className={`flex items-center space-x-1 xxs-sm:space-x-2`}>
                                            ...
                                            >
                                                <Minus
                                                    size={14}
                                                    className='xxs-sm:size-[16px] h-[9px] w-[9px] xxs-sm:h-[10px] 
                                                        xxs-sm:w-[10px] text-secondary'
                                                />
                                            </button>
                                            <span
                                                className={` text-gray-800 font-[500] ${retractedView 
                                                    ? "text-[12px] xxs-sm:text-[13px]" 
                                                    : "text-[11px] xxs-sm:text-[12px]"}`}
                                            >
                                                {product.quantity}
                                            </span>
                                            ...
                                            >
                                                <Plus
                                                    size={14}
                                                    className='xxs-sm:size-[16px] h-[9px] w-[9px] xxs-sm:h-[10px] xxs-sm:w-[10px]
                                                        text-secondary'
                                                />
                                            </button>
                                            ...
                                            {!retractedView && (
                                                <button
                                                    ...
                                                    className='text-red-500 hover:text-red-700 ml-1 xxs-sm:ml-2'
                                                >
                                        ...
                                        {product?.offerApplied && product?.offerDiscount && (
                                            <p
                                                className={` ${!retractedView && "mt-[8px] xxs-sm:mt-[10px] w-full"} px-[4px] 
                                                    xxs-sm:px-[5px] py-[2px] bg-inputBgSecondary flex items-center
                                                    gap-[2px] xxs-sm:gap-[3px] text-[9px] xxs-sm:text-[10px] text-secondary 
                                                    border border-inputBorderSecondary rounded-[4px] 
                                        ...
                                        <i className='absolute top-[50%] left-[-3px] p-[4px] xxs-sm:p-[5px] bg-white 
                                                border border-dropdownBorder border-l-white rounded-[3px] 
                                                shadow-sm z-[20] cursor-pointer'
                                            onClick={() => setScaleDownSidebar(true)}
                                        >
                    ...
                    {Object.keys(packedupCart).length > 0 &&
                        packedupCart?.products &&
                        packedupCart.products.length > 0 &&
                        !scaleDownSidebar && (
                            <div className='border-t p-3 xxs-sm:p-4 bg-white sticky bottom-0'>
                                <div
                                    className={`flex justify-between items-center font-bold 
                                        ${retractedView ? "text-[10px] xxs-sm:text-[11px]" : "text-base xxs-sm:text-lg"} 
                                        text-gray-800 mb-2 xxs-sm:mb-3`}
                                >
                                    <span>Total:</span>
                                    <span>
                                        &#8377;{" "}
                                        {packedupCart.absoluteTotal ? packedupCart.absoluteTotal.toFixed(2) : null}{" "}
                                    </span>
                                </div>
                                ...
                                {!retractedView && (
                                    <SiteSecondaryFillImpButton
                                        ...
                                        className='w-full text-[13px] xxs-sm:text-base py-2 xxs-sm:py-2.5 mb-2'>
                                        Proceed to Checkout
                                    </SiteSecondaryFillImpButton>
                                )}
                                ...
                                <SiteSecondaryFillImpButton
                                    ...
                                    customStyle={
                                        retractedView
                                            ? { paddingBlock: "3px", fontSize: "12px xxs-sm:text-[13px]" }
                                            : {}
                                    }>
                                    Go To Cart
                                </SiteSecondaryFillImpButton>
