...
export const createNewAddress = createAsyncThunk("createNewAddress", async ({ id, addressData }, thunkAPI) => {
    try {
        ...
        const errorMessage =
            error.response?.data?.message || error.message || "Something went wrong while adding new address. Please try again later."
...
export const getAllAddress = createAsyncThunk("getAllAddress", async (_, thunkAPI) => {
    try {
        ...
        const errorMessage =
            error.response?.data?.message ||
            error.message ||
            "Something went wrong while loading address. Please try again later."
        return thunkAPI.rejectWithValue(errorMessage)
    }
})
...
export const setAsDefaultAddress = createAsyncThunk("setAsDefaultAddress", async ({ addressId }, thunkAPI) => {
    try {
        ...
        const errorMessage =
            error.response?.data?.message ||
            error.message ||
            "Something went wrong while setting default address. Please try again later."
        return thunkAPI.rejectWithValue(errorMessage)
    }
})
