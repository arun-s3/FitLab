...
export default function AiInsightCards({
    ...
    cardStyles,
    sourceDatasLoading,
    excludeAndReturnItemTitles = null,
    onReturnExclusiveDatas,
    parentFetchError = null,
    guestMessage = null,
}) {
    ...
    useEffect(() => {
        ...
        if (insightsTemplates && insightsTemplates.length > 0 && existingAiInsights && !hasCreatedCards.current) {
            ...
            insightsTemplates &&
            insightsTemplates.length > 0 &&
            requiredSourceDatas &&
            !hasCreatedCards.current
        ) {
            hasCreatedCards.current = true
            getAiInsights()
        }
            ...
            <div className='w-full bg-whitesmoke mt-6 max-w-7xl mx-auto py-8 sm:px-6 border border-dropdownBorder rounded-xl'>
                ...
                >
                    <div>
                        <div className='flex items-center gap-3 mb-3'>
                            ...
                            <div>
                                <h2 className='text-[21px] font-bold text-gray-900 dark:text-white'>
                                    {" "}
                                    {sectionTitle}{" "}
                                </h2>
                                <p className='-mt-[3px] text-[14px] text-gray-600 dark:text-gray-400'>
                                    {" "}
                                    {sectionSubtitle}{" "}
                                </p>
                            </div>
                        ...
                        <div className='flex gap-4 items-center'>
                            <p className='text-gray-600 dark:text-gray-400'>
                                Our AI is analyzing your workout. This process may take up to a minute...
                            </p>
                        </div>
                    </div>
                    <div className='mr-8'>
                ...
                <div className='relative grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>
                    {Array.from({ length: insightsTemplates.length }).map((_, i) => (
                        <div
                            key={i + 1}
                            className={`bg-white min-h-[19rem] dark:bg-gray-800 rounded-2xl p-6 border 
                                border-gray-200 dark:border-gray-700 ${i + 1 === 5 && "col-span-2"}`}
                        >
    ...
    return (
        <div className='w-full bg-whitesmoke mt-6 max-w-7xl mx-auto py-8 sm:px-6 border border-dropdownBorder rounded-xl'>
            ...
            <motion.div
                ...
                animate='visible'
                className={`${
                    parentFetchError || error || (!user && !admin && guestMessage)
                        ...
                        return (
                            ...
                            >
                                <motion.div
                                    ...
                                    className={` relative overflow-hidden min-h-[19rem] bg-white dark:bg-gray-800
                                          rounded-2xl p-6 border-2 transition-colors duration-300
                                          ${index === 5 && "col-span-2"}
                                          ${
                                              isSelected
