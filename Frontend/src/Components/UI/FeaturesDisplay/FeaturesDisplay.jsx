...
export default function FeaturesDisplay({ darkVariant = false, topRoom = true }) {
    ...
    return (
        <div
            className={`mb-[1rem] grid grid-cols-1 md:grid-cols-3 gap-[2rem] ${topRoom && "mt-[4rem]"} p-[2rem]
            ...
            {features && features.length > 0 &&
                features.map((feature) => (
                    <div key={feature.title} className='text-center'>
                        <div className='w-[4rem] h-[4rem] bg-purple-600 rounded-full flex items-center 
                            justify-center mx-auto mb-[1rem]'
                        >
                            <svg
                        ...
                        <h3 className={`font-bold mb-[8px] ${darkVariant ? "text-white" : "text-black"}`}>
                            {" "}
                            {feature.title}{" "}
                        </h3>
                        <p
                            className={`text-[12px] xs-sm:text-[14px] ${darkVariant ? "text-[#cdc8c8]" : "text-gray-600"}`}>
                            {feature.description}
                        </p>
                    </div>
                ))}
