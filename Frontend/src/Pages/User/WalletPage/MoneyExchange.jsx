...
export default function MoneyExchange({ walletBalance, onAuthCheckModal }) {
        ...
        () => {
            if (Object.values(error).some((val) => val)) {
                const timeout = setTimeout(() => {
                    setError({ sendMoney: "", receiveMoney: "" })
                }, 2000)
                return () => clearTimeout(timeout)
            }
            if (Object.values(inputMsg).some((val) => val)) {
                const msgTimeout = setTimeout(() => {
                    setInputMsg({ sendMoney: "", receiveMoney: "" })
                }, 700)
    ...
    const handlePasteFromClipboard = async (type) => {
        ...
        try {
            ...
            const setAccNo = (no) =>
                type === "sendMoney"
                    ? setSelectedRecipient({ name: "", accountNumber: no })
    ...
    return (
        ...
        >
            <div className='mt-0 lg:mt-8 xl:mt-0 flex flex-col gap-[10px] lg:gap-12 xl:gap-[10px]'>
                <motion.div className='bg-white shadow-sm rounded-lg'>
                    <div className='pt-[1.2rem] pb-[1rem] px-[1rem] mob:px-[1.5rem]'>
                        ...
                        <div className='relative flex flex-wrap gap-2 mb-[6px]'>
                            <input
                                ...
                                className='flex-1 min-w-[100px] xxs-sm:min-w-[140px] h-[2rem] px-2 mob:px-3 py-2 text-[11px] 
                                    mob:text-[12px] border border-gray-300 rounded-md focus:outline-none focus:ring-2 
                                    focus:ring-purple-500 focus-visible:outline-none focus:border-secondary'
                                onChange={(e) => {
                            ...
                            <motion.button
                                className='h-[2rem] w-8 mob:w-10 flex items-center justify-center border border-gray-300 
                                    rounded-md hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-purple-500 
                                    hover:text-secondary hover:border-secondary hover:border-2 transition duration-300'
                                whileTap={{ scale: 0.95 }}
                    ...
                    <div className='pt-[13px] pb-[1rem] px-[1rem] mob:px-[1.5rem]'>
                        ...
                        <div className='relative flex flex-wrap gap-2 mb-[6px]'>
                            ...
                            <input
                                type='text'
                                placeholder='Enter his/her FitLab account number'
                                className='flex-1 min-w-[8px] mob:min-w-[100px] xx-lg:min-w-[140px] h-[2rem] px-2 mob:px-3 py-2 
                                    text-[11px] mob:text-[12px] border border-gray-300 rounded-md focus:outline-none focus:ring-2 
                                    focus:ring-purple-500 focus:border-secondary'
                                onChange={(e) => {
                                    const value = e.target.value.replace(/[^\d\s]/g, "")
