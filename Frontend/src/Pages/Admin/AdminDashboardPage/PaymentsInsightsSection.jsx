    ...
    return (
        ...
        >
            <h2 className='text-xl text-secondary font-bold flex items-center gap-[10px]'>
                <span className={`w-fit whitespace-nowrap ${!showOperationsAnalytics.payments && "text-muted"}`}>
                    {" "}
                    Payments & Refunds{" "}
                </span>
                <div
                    ...
                    onClick={() =>
                        togglerEnabled &&
                        setShowOperationsAnalytics((status) => ({ ...status, payments: !status.payments }))
                    }>
                    <hr
                        ...
                        <div className='grid grid-cols-1 lg:grid-cols-2 gap-6'>
                            <motion.div
                                ...
                                className={`bg-white dark:bg-gray-800 p-6 rounded-lg shadow-sm border
                                    ${dashboardQuery.trim() 
                                        && !"Payment Methods Usage".toLowerCase().trim().includes(dashboardQuery.trim()) 
                                        ? "hidden" 
                                        : "inline-block"
                                    }`}
                            >
                                ...
                                <div className={`${status.paymentMethodsDatas !== "error" ? "h-64" : "h-40"} relative`}>
                                    {status.paymentMethodsDatas !== "loading" ? (
                                        <ResponsiveContainer width='100%' height='100%'>
                                            ...
                                            {status.paymentMethodsDatas !== "error" &&
                                                paymentMethodsDatas &&
                                                (paymentMethodsDatas.length === 0 ||
                                                    paymentMethodsDatas.every((data) => data.value === 0)) && (
                                                    <p
                                                        className='absolute top-0 w-full h-full flex items-center 
                                                            justify-center text-muted  text-[13px] sm:text-[16px] 
                            ...
                            <motion.div
                                ...
                                className={`bg-white dark:bg-gray-800 p-6 rounded-lg shadow-sm border
                                    ${dashboardQuery.trim() 
                                        && !"Refund Requests".toLowerCase().trim().includes(dashboardQuery.trim()) 
                                        ? "hidden" 
                                        : "inline-block"
                                    }`}
                            >
                                ...
                                <div className={`${status.refundRequestDatas !== "error" ? "h-64" : "h-40"} relative`}>
                                    {status.refundRequestDatas !== "loading" ? (
                                        <ResponsiveContainer width='100%' height='100%'>
                                            ...
                                            {status.refundRequestDatas !== "error" &&
                                                refundRequestDatas &&
                                                (refundRequestDatas.length === 0 ||
                                                    refundRequestDatas.every(
                                                        (data) => data.requests === 0 && data.amount === 0,
                                                    )) && (
                                                    <p
                                                        className='absolute top-[8.7rem] w-full h-full flex items-center 
