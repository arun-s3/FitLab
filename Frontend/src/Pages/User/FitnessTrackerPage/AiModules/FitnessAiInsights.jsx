...
export default function FitnessAiInsights({ receivedSourceDatas, periodType, guestMode = false }) {
    ...
    const getInsightDataSources = async () => {
        ...
        try {
            ...
            setError(
                "Something went wrong while finding your tracking history! Please check your network and retry later",
            )
        } finally {
            setLoading(false)
        }
    ...
    useEffect(() => {
        if (generateAiInsights && receivedSourceDatas) {
            ...
            const everyInsightDatatypesExists = insightDatatypes.every((datatype) =>
                Object.keys(receivedSourceDatas).includes(datatype),
            )
            if (everyInsightDatatypesExists) {
                const fitnessDatas = {
                    analysisType: "periodFitnessProfile",
                    fitnessProfile: receivedSourceDatas,
                    periodType,
                }
