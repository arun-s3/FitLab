...
export default function BreadcrumbBar({ heading }) {
    ...
    const breadcrumbPath = pathArray.map((currentCrumb) => {
        ...
        return (
            <Link to={breadCrumb} key={currentCrumb} className='flex items-center'>
                <h4
                    className='hidden xs-sm:inline-block text-white capitalize max-xs-sm2:text-[14px] hover:text-primary
                        hover:font-[500] hover:underline'
                >
                    {currentCrumb}
                </h4>
                <h4
                    className='inline-block xs-sm:hidden text-white capitalize max-xs-sm:text-[14px] hover:text-primary 
                        hover:font-[500] hover:underline'
                >
                    {currentCrumb == "Home" ? (
                ...
                {pathArray.indexOf(currentCrumb) == pathArray.length - 1 ? null : (
    ...
    return (
        <div
            style={barBg}
            className='mt-[5px] h-[5rem] max-xs-sm:h-[4rem] max-xs-sm:pl-[1.8rem] pl-[60px] flex flex-col 
        ...
        >
            ...
            <h1
                className='hidden sm:inline-block text-[18px] font-[600] l-md:text-breadcrumbTitle capitalize 
                    trackig-[0.5px] text-white z-[5]'
