    ...
    function HistoryCard({ workout, date = null, allowResumeExercise = false, workoutCompleted }) {
        ...
        return (
            <motion.div
                ...
                className='rounded-lg p-4 bg-purple-50 border border-gray-200 hover:border-purple-300 hover:shadow-md 
                    transition-all'
            >
                ...
                {allowResumeExercise && !workoutCompleted && (
                    <motion.div
                        whileTap={{ scale: 0.98 }}
                        className='mt-[10px] ml-[3px] flex items-center justify-between gap-[10px]'
                    >
                        <button
                        ...
                        >
                            {loadingResume ? (
                                ...
                                <>
                                    {selectedWorkoutId !== workout._id && <Play className='w-[17px] h-[17px]' />}
                                    <span>{selectedWorkoutId === workout._id ? "Resumed" : "Resume Exercise"}</span>
                                </>
                            )}
                        </button>
                ...
                {!loading && olderWorkouts && olderWorkouts.length > 0 ? (
                    olderWorkouts.map((workouts) =>
                        workouts.exercises.map((exercise) => (
                            <HistoryCard key={exercise?._id || index} workout={exercise} date={workouts.date} />
                        )),
                    )
