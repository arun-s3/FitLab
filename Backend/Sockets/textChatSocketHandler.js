        ...
        io.on("connection", (socket) => {
            ...
            socket.on("user-login", async (userData) => {
                ...
                if (!usernameJoined) {
                    ...
                    const usernameJoinedButOffline = joinedUserDatas.some(
                        (data) => data.username === userData.username && !data.isOnline,
                    )
                    if (usernameJoinedButOffline) {
                        const activeButOfflineUser = joinedUserDatas.find(
                            (data) => data.username === userData.username && !data.isOnline,
                        )
            ...
            socket.on("load-chat-history", async ({ roomId, page = 0, limit = 15, isAdmin }) => {
                try {
                    const messages = await TextChat.find({ roomId })
                        .sort({ createdAt: -1 })
                        .skip(page * limit)
                        .limit(limit)
