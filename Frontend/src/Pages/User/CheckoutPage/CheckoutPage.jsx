...
export default function CheckoutPage() {
    ...
    useEffect(() => {
        if (productRemoved) {
            const unavailableProducts = cart.products.filter(
                (product) =>
    ...
    const checkProductsAvailability = () => {
        return cart.products.every((product) => {
            return product.productId && !product.productId.isBlocked && product.productId.stock >= product.quantity
        })
    }
    ...
    const placeOrder = async () => {
        ...
        try {
            if (paymentMethod === "razorpay") {
                ...
                try {
                    const response = await apiClient.post(`/wallet/order`, {
                        amount: cart.absoluteTotalWithTaxes.toFixed(2),
                    })
        ...
        <section id='CheckoutPage'>
            ...
            <main className='mb-[7rem]'>
                ...
                <div className='my-[4rem]'>
                    {cart?.products && cart.products.length > 0 ? <OrderStepper stepNumber={2} /> : null}
                </div>
                ...
                {cart?.products && cart.products.length > 0 ? (
                    ...
                    >
                            <motion.div className='flex flex-col gap-[20px]' variants={containerVariants}>
                                ...
                                <motion.div
                                    ...
                                    <PaymentsManager
                                        ...
                                        cartTotal={
                                            cart && cart.absoluteTotalWithTaxes ? cart.absoluteTotalWithTaxes : null
                                        }
                        ...
                        {cart?.products && (
                            <motion.div variants={stepVariants} initial='hidden' animate='show'>
                                <OrderSummary
                                    shippingAddress={shippingAddress}
                                    paymentMethod={paymentMethod}
                    ...
                    >
                        <p className='hidden xx-md:inline-block text-[17px] text-muted capitalize tracking-[0.5px]'>
                            ...
                            <Link
                                to='/shop'
                                className='mx-[5px] text-secondary hover:underline transition duration-300 cursor-pointer'>
                                here
                            </Link>
                            to return to the shop and continue shopping!
                        </p>
                        <div className='xx-md:hidden text-muted capitalize tracking-[0.5px]'>
                            <p className='text-center text-[15px] s-sm:text-[17px]'>
                                {" "}
                ...
                {openChatBox && (
                    <div className='fixed bottom-[2rem] right-[2rem] z-50'>
                        <TextChatBox closeable={true} openChats={true} onCloseChat={() => setOpenChatBox(false)} />
                    </div>
                )}
            </main>
