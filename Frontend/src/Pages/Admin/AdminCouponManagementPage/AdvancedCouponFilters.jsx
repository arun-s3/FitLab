...
export default function AdvancedCouponFilters({ queryOptions, setQueryOptions, close }) {
    ...
    useEffect(() => {
        ...
        if (hasFilters) {
            ...
            if (queryOptions?.applicableType) {
                const applicableTypeLabel = allApplicableTypes.find(
                    (type) => type.value === queryOptions.applicableType,
                ).name
                setApplicableType((type) => ({
                    ...type,
                    name: applicableTypeLabel,
                    value: queryOptions.applicableType,
                }))
            }
    ...
    const submitFilters = () => {
        setQueryOptions((query) => ({
            ...
            applicableType: applicableType.value,
        }))
    ...
    return (
        <main
            className='w-[20rem] bg-white pt-[20px] pb-[25px] px-[35px] border border-borderLight2 rounded-[8px]'
            id='advancedCouponFilters'
        ...
        >
            ...
            <div className='flex flex-col gap-[1rem] mt-[1rem]' id='filters'>
                <div>
                    ...
                    <div
                        className='relative w-[16rem] h-[30px] px-[10px] py-[8px] text-[14px] text-muted bg-white flex items-center 
                        ...
                        onClick={() =>
                            setDiscountType((discountType) => ({ ...discountType, show: !discountType.show }))
                        }
                    >
                        ...
                        {discountType.show && (
                            <ul
                                className='list-none  px-[10px] py-[1rem] absolute top-[37px] right-0 flex flex-col gap-[10px] 
                                    justify-center w-[16rem] text-[10px] bg-white border border-borderLight2 rounded-[8px] 
                                    z-[5] cursor-pointer'
                                onMouseLeave={() =>
                                    setDiscountType((discountType) => ({ ...discountType, show: false }))
                                }
                            >
                                {allDiscountTypes &&
                                    allDiscountTypes.map((type) => (
                                        ...
                                        >
                                            <span
                                                className={`w-full h-[26px] pl-[8px] flex items-center gap-[7px] rounded-[5px] 
                                                    hover:text-primaryDark hover:bg-[#F5FBD2] 
                                                    ${discountType.value === type ? "text-primaryDark" : "text-muted"}}`}
                                                onClick={() => setDiscountType({ show: false, value: type })}
                                            >
                                                {(() => {
                ...
                <div>
                    <div>
                        ...
                        <div
                            className='relative w-[16rem] h-[30px] px-[10px] py-[8px] text-[14px] text-muted bg-white 
                                flex items-center gap-[10px] justify-center border border-dropdownBorder rounded-[4px] 
                                shadow-sm cursor-pointer hover:bg-gray-50 transition-colors optionDropdown sort-dropdown'
                        ...
                        >
                            <div className='w-full flex items-center justify-between'>
                                <div className='flex items-center gap-[5px]'>
                                    ...
                                    <span className='text-[12px] font-[470] text-secondary'>{applicableType.name}</span>
                                </div>
                                {applicableType.show ? (
                                    <ChevronUp className='h-[15px] w-[15px] text-muted' />
                                ) : (
                            ...
                            {applicableType.show && (
                                <ul
                                    className='list-none  px-[10px] py-[1rem] absolute top-[37px] right-0 flex flex-col 
                                        gap-[10px] justify-center w-[16rem] text-[10px] bg-white border 
                                ...
                                >
                                    {allApplicableTypes &&
                                        allApplicableTypes.map((type) => (
                                            ...
                                            >
                                                <span
                                                    className={`w-full h-[26px] pl-[8px] flex items-center gap-[7px] 
                                                        rounded-[5px] hover:text-primaryDark hover:bg-[#F5FBD2]
                                                        ${applicableType.value === type.value ? "text-primaryDark" : "text-muted"}}`}
                                                >
                                                    {(() => {
                ...
                <div>
                    <div className='relative'>
                        <div className='w-full flex items-center justify-between'>
                            ...
                            <span
                                className={`text-[10px] ${isDisabled.usedCount ? "text-[#07bc0c]" : "text-red-500"}`}
                                onClick={() =>
                                    setIsDisabled((isDisabled) => ({ ...isDisabled, usedCount: !isDisabled.usedCount }))
                                }
                            >
                        ...
                        <input
                            ...
                            name='usedCount'
                            value={usedCount}
                            placeholder='Filter offers below this used count'
                            disabled={isDisabled.usedCount}
                            className={`h-[2.2rem] ${isDisabled.usedCount && "bg-[#E9EBEE]"}`}
                            onBlur={(e) => inputBlurHandler(e, "usedCount")}
                        ...
                        <span className='error bottom-[-15px] right-0'> {error.usedCount && error.usedCount} </span>
                    </div>
                </div>
                ...
                <div className='mt-[5px] mb-[10px]'>
                    <div className='relative'>
                        <div className='w-full flex items-center justify-between'>
                            ...
                            <span
                                className={`text-[10px] ${isDisabled.minimumOrderValue ? "text-[#07bc0c]" : "text-red-500"}`}
                                onClick={() =>
                                    setIsDisabled((isDisabled) => ({
                                        ...isDisabled,
                                        minimumOrderValue: !isDisabled.minimumOrderValue,
                                    }))
                                }
                            >
                        ...
                        <input
                            ...
                            placeholder='Filter offers below this minimum order value'
                            disabled={isDisabled.minimumOrderValue}
                        ...
                        <span className='error bottom-[-15px] right-0'>
                            {" "}
                ...
                <div className='text-right'>
                    <SiteButtonSquare
                        customStyle={{ paddingBlock: "3px", paddingInline: "28px", borderRadius: "4px" }}
                        light={true}
                        lowFont={true}
                        clickHandler={() => submitFilters()}>
                        Apply
                    </SiteButtonSquare>
                </div>
