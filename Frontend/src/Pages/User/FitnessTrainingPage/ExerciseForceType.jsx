...
export default function ExerciseForceType({ exercise }) {
    ...
    const getMovementType = (exerciseName = "") => {
        ...
        const staticKeywords = ["plank", "hold", "isometric", "wall sit", "dead bug", "bird dog", "bridge"]
        if (staticKeywords.some((k) => n.includes(k))) return "static"

        const pullKeywords = ["pull", "row", "curl", "shrug", "chin", "lat", "face pull", "pulldown"]
            ...
            {force && (
                <>
                    ...
                    <div>
                        ...
                        <p className='text-sm font-semibold text-slate-900 capitalize'>{force || "Mixed"}</p>
                    </div>
                </>
            )}
        </div>
