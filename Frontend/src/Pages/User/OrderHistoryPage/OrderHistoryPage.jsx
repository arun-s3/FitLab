...
export default function OrderHistoryPage() {
    ...
    useEffect(() => {
        if (canceledReturnRequest) {
            sonnerToast.success(
                "The return request has been declined, and the return process has been permanently closed!",
            )
            dispatch(resetOrderStates())
        }
    ...
    const cancelOrReturnProduct = (e, productId, orderId, orderDate) => {
        if (e.target.textContent.toLowerCase().includes("cancel")) {
            setOpenCancelForm({
                type: "product",
                status: true,
                options: { productId: productId, orderId },
                return: false,
            })
    ...
    const cancelReasonHandler = (e, options) => {
        if (options.title) {
            setOpenSelectReasons((selectReason) => ({ ...selectReason, reasonTitle: e.target.textContent }))
        }
        if (options.content) {
    ...
    const confirmCancelReturn = () => {
        if (cancelReturnReq.cancelType) {
            const returnDetails = {
                orderId: cancelReturnReq.orderId,
                productId: cancelReturnReq.productId._id,
                returnType: cancelReturnReq.cancelType,
            }
    ...
    const handleAddReview = async (newReview, isTestimony, id) => {
        try {
            ...
            if (response?.data?.success) {
                sonnerToast.success(
                    `Your ${showReviewForm.testimony ? "testimony" : "review"} has been submitted successfully!`,
                )
                    ...
                    <div className='max-w-[90rem] mx-auto'>
                        <div className='mb-[2rem] flex items-center justify-between' id='orderHistory-header'>
                            <h1 className='text-[25px] font-bold flex items-center gap-2 text-gray-800'>
                                Your Orders
                                <span
                                    className='px-[12px] py-[4px] text-[13px] text-white font-[500] bg-gradient-to-r
                                    ...
                                    {orders.map(
                                        (order) =>
                                            ...
                                            !order?.products.every((product) => product.isDeleted) && (
                                                <motion.div
                                                    key={order._id}
                                                    className='pb-[1rem] bg-white rounded-2xl shadow-sm hover:shadow-md 
                                                ...
                                                >
                                                    ...
                                                    {!noShowRating.includes(order._id) &&
                                                    ...
                                                    {!noShowRating.includes(order._id) &&
                                                        ...
                                                        fitlabRating && (
                                                            ...
                                                            >
                                                                ...
                                                                <ReviewForm
                                                                    onSubmit={(newReview) =>
                                                                        handleAddReview(newReview, true, order._id)
                                                                    }
                                                    ...
                                                    <div className='p-[1.5rem] pb-0 flex items-center justify-between'>
                                                        <div className='flex items-center gap-[3rem] basis-[40%]'>
                                                            ...
                                                            <h3
                                                                className={`px-[5px] py-[3px] 
                                                                    ${order.orderStatus === "returning" ? "w-[35%]" : "w-[23%]"} 
                                                                    flex gap-[15px] justify-center items-center
                                                                    ${findStyle(order.orderStatus, "lightBg")} border 
                                                                    ${findStyle(order.orderStatus, "border")} rounded-[5px]`}
                                                            >
                                                                <span
                                                                    className={`w-[8px] h-[8px] ${findStyle(order.orderStatus, "bg")} 
                                                                        rounded-[5px]`}
                                                                    style={{
                                                                        boxShadow: `0px 0px 6px 3px ${findStyle(order.orderStatus, "shadow")}`,
                                                                    }}
                                                                >
                                                                </span>
                                                                <span
                                                                    className={`capitalize ${findStyle(order.orderStatus, "textColor")} 
                                                                        text-[14px]`}
                                                                >
                                                                    {order.orderStatus === "returning"
                                                                        ? "Appied for return"
                                                        ...
                                                        {openCancelConfirmModal && (
                                                            ...
                                                            <Modal
                                                                ...
                                                                setOpenModal={setOpenCancelConfirmModal}
                                                                title='Important'
                                                                ...
                                                                >
                                                                        ...
                                                                        {openCancelForm.type === "product" &&
                                                                            ...
                                                                            openCancelForm.status && (
                                                                                ...
                                                                                <CancelForm
                                                                                    ...
                                                                                    formFor='product'
                                                                                    returnAndRefund={ openCancelForm.return }
                                                                                />
                                                                    ...
                                                                    {product &&
                                                                        ...
                                                                        product?.productStatus === "delivered" && (
                                                                            ...
                                                                            >
                                                                                <div
                                                                                    className='p-[11px] text-[13px] font-[500] 
                                                                                        flex justify-between items-center bg-gradient-to-r
                                                                                        from-yellow-50 to-orange-50 border
                                                                                        border-yellow-100 rounded-xl'
                                                                                >
                                                                                    <div className='flex items-center gap-[8px]'>
                                                                                        <Star className='w-[20px] h-[20px] 
                                                                                            text-yellow-400 fill-current' />
                                                                                        <span className='font-[13px] cursor-pointer
                                                    ...
                                                    {openRefundModal.status && (
                                                        ...
                                                        <RefundModal
                                                            isOpen={openRefundModal.status}
                                                            onClose={() =>
                                                                setOpenRefundModal((info) => ({
                                                                    ...info,
                                                                    status: false,
                                                                }))
                                                            }
                                                            returnOrderOrProduct={initiateReturnProduct}
                                                            orderOrProduct={openRefundModal.orderOrProduct}
                                                            onReasonWritten={setOpenSelectReasons}
                                                            reasonWritten={openSelectReasons}
                                                        />
                                                    ...
                                                    {showOrderDetailsModal.status && (
                                                        ...
                                                        <OrderDetailsModal
                                                            order={showOrderDetailsModal.order}
                                                            isOpen={showOrderDetailsModal.status}
                                                            onClose={() =>
                                                                setShowOrderDetailsModal({ status: false, order: null })
                                                            }
                                                        />
