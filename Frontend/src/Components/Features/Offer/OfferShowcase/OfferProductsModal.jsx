        ...
        const productsWithoutVariants = applicableProducts
            ...
            .map((parent) => {
                const variantPrices = parent.variants
                    .map((variantId) =>
                        applicableProducts.find((product) => product._id.toString() === variantId.toString()),
                    )
                    .filter(Boolean)
                    ...
                    <div className='p-6'>
                        <AnimatePresence mode='wait'>
                            ...
                            >
                                ...
                                <div>
                                    <h4
                                        className='text-lg capitalize font-bold text-slate-900 dark:text-white 
                                            hover:underline hover:underline-offset-[5px] decoration-secondary transition 
                                            duration-300 mb-2 cursor-pointer'
                                        onClick={() =>
                                            navigate(
                                                {
                                                    pathname: "/shop/product",
                                    ...
                                    <p className='text-slate-600 dark:text-slate-400 text-sm mb-3 line-clamp-2'>
                                        {capitalizeFirstLetter(products[currentIndex]?.subtitle) ||
                ...
                {products.length > 1 && (
                    <div className='flex items-center justify-between p-6 border-t border-slate-200 gap-4'>
                        <motion.button
                            ...
                            className='flex-1 flex items-center justify-center gap-2 bg-slate-200
                                hover:bg-slate-300 text-slate-900 py-2 rounded-lg transition-colors font-medium'
                        >
