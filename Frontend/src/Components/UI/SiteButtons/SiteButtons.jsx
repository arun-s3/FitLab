...
export function SiteButton({ customStyle, children, className, clickHandler, shouldSubmit = false }) {
    ...
    return (
        <button
            ...
            className={`bg-primary text-black text-descReg1 hover:bg-yellow-300 transition 
                duration-300 ${className} site-button`}
            style={customStyle}
...
export function SiteButtonDark({ customStyle, children, className = "", shouldSubmit = false }) {
    ...
    return (
        <button
            type={shouldSubmit ? "submit" : "button"}
            className={`bg-black text-white text-descReg1 
                site-button-dark ${className}`
            }
            style={customStyle}
        >
            {children}
...
export function SiteButtonSquare({
    customStyle,
    tailwindClasses,
    light,
    lighter,
    ...
    return (
        ...
        <button
            ...
            className={`${tailwindClasses ? tailwindClasses : ""} site-button-square bg-primary text-black 
                !disabled:cursor-not-allowed text-descReg1 
                ${light ? "font-[480]" : lighter ? "font-[450]" : ""}`}
            style={computedStyle}
            onClick={clickHandler ? () => clickHandler() : undefined}
...
export function SiteSecondaryBorderButtonSquare({
    customStyle,
    tailwindClasses,
    children,
    clickHandler,
...
export function SitePrimaryWhiteTextButton({
    customStyle,
    tailwindClasses,
    light,
    lowFont,
...
export function SitePrimaryButtonWithShadow({
    customStyle,
    tailwindClasses,
    animated,
    noHover = false,
            ...
            {animated ? (
                <motion.button
                    ...
                    className={`inline-flex justify-center items-center gap-[5px] py-[4px] px-[15px] rounded-[8px]
                        text-[14px] text-[#332929] tracking-[0.2px] bg-primary 
                        ${!noHover && "hover:bg-primaryDark"} cursor-pointer 
                ...
                <button
                    type={shouldSubmit ? "submit" : "button"}
                    className={`inline-flex justify-center items-center gap-[5px] py-[4px] px-[15px] rounded-[8px]
                        text-[14px] text-[#332929] tracking-[0.2px] bg-primary cursor-pointer 
                        ${tailwindClasses ? tailwindClasses : ""} ${light ? "font-[480]" : ""}`}
                    style={lowFont ? { ...customStyle, fontSize: "14px" } : customStyle}
                    onClick={clickHandler ? () => clickHandler() : undefined}
                    id='SitePrimaryButtonWithShadow'
...
export function SitePrimaryMinimalButtonWithShadow({
    customStyle,
    tailwindClasses,
    light,
    lowFont,
    ...
    return (
        <button
            type={shouldSubmit ? "submit" : "button"}
            className={` flex justify-center items-center gap-[3px] px-[16px] py-[2px] rounded-[4px]
...
export function SiteSecondaryFillButton({
    className,
    variant,
    size,
    clickHandler,
    ...
    return (
        <button
            className={`px-[16px] py-[8px] rounded
...
export function SiteSecondaryFillImpButton({
    className,
    variant,
    clickHandler,
    isDisabled,
    ...
    return (
        <button
            className={`w-full mt-4 py-3 px-4 rounded-lg font-semibold transition duration-300
                ${
                     variant === "outline"
                         ? `bg-white border-[2px] text-secondary border-secondaryLight2 hover:text-white 
