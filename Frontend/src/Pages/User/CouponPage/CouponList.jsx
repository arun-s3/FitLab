...
export default function CouponList({ coupons, onCouponApply }) {
    ...
    return (
        <div
            className={`${
                coupons.length > 0
                    ? "mr-8 xx-md:mr-auto grid grid-cols-1 xx-md:grid-cols-2 x-xl:grid-cols-3 gap-6"
                    ...
                    >
                        <div className='p-4 bg-gray-50'>
                            <div className='flex justify-between items-start'>
                                ...
                                <span className='px-[10px] py-[5px] bg-purple-100 text-secondary text-xs font-semibold 
                                    rounded-full flex items-center'
                                >
                                    {(() => {
                                        const Icon = discounTypeIcons.find(
                                            (type) => type.name === coupon.discountType,
                                        ).Icon
                        ...
                        <div className='p-4 flex-grow'>
                            ...
                            <div className='space-y-2 text-sm'>
                                ...
                                <p className='flex items-center'>
                                    ...
                                    <span
                                        className={`ml-[5px] 
                                            ${coupon.usedCount < coupon.usageLimitPerCustomer ? "text-green-500" : "text-red-500"}`}>
                                ...
                                <p className='relative flex items-center'>
                                    ...
                                    <span 
                                        className={`text-[13px] text-secondary hover:underline transition duration-300 
                                            ease-in cursor-pointer ${coupon.applicableType === "allProducts" && "hidden"}`}
                                        onMouseEnter={() =>
                                            setShowItemsOf({
                                                type:
                                                    coupon.applicableType === "products"
                                            ...
                                            >
                                                {coupon[showItemsOf.type].map((categoryOrProduct) => (
                                                    <li key={categoryOrProduct._id} className='capitalize text-muted'>
                                                        {(() => {
                                                            const name =
                                                                showItemsOf.type === "applicableProducts"
                                                                    ? categoryOrProduct.title
                                                                    : categoryOrProduct.name
