...
const getAnnualRevenueStats = async (req, res, next) => {
    try {
        ...
        const revenueChangeDay =
            yesterdayRevenue === 0 ? null : ((todayRevenue - yesterdayRevenue) / yesterdayRevenue) * 100
...
const getTotalOrdersCount = async (req, res, next) => {
    try {
        ...
        const changePercentage =
            yesterdayTotalOrders === 0 ? null : ((totalOrders - yesterdayTotalOrders) / yesterdayTotalOrders) * 100
        return res.status(200).json({
            success: true,
            totalOrders,
