...
export default function CouponList({ coupons, onEdit, onDelete, onDeactivate, onSort, sortConfig }) {
    ...
    const tableHeaders = [
        { value: "Code", icon: true, sortBy: "code" },
        { value: "Description", icon: false },
        { value: "Discount", icon: false },
        { value: "Applicable To", icon: false },
        { value: "Start Date", icon: true, sortBy: "startDate" },
        { value: "Expiry", icon: true, sortBy: "endDate" },
        { value: "Used/ Usage-limit", icon: true, sortBy: "usageLimit" },
    ...
    return (
        <div className={`mt-[1.5rem] overflow-hidden 
                ${coupons.length > 0 ? "bg-white shadow-md rounded-lg" : "flex justify-center items-center"}`}
            id='CouponList'
        >
            {coupons.length > 0 ? (
                <table className='min-w-full divide-y divide-gray-200 table-fixed'>
                    <thead className='bg-inputBorderLow'>
                        <tr>
                            {tableHeaders.map((header) => (
                                <th
                                    key={header.value}
                                    className='px-[12px] py-3 text-left text-[13px] font-medium text-gray-500 uppercase tracking-wider
                                        cursor-pointer'
                                    onClick={() => {
                                ...
                                >
                                    <div className='flex items-center'>
                                        {header.value}
                                        {header.icon && (
                                            <ArrowUpDown className='h-4 w-4 ml-1 hover:text-purple-700 hover:scale-110 transition duration-100' />
                                        )}
                                    </div>
                                </th>
                            ))}
                    ...
                    <tbody className='bg-white divide-y divide-gray-200'>
                        {coupons.map((coupon, index) => (
                            <tr
                                key={coupon?._id}
                                className={`${index % 2 == 0 ? "bg-transparent" : "bg-[#eee]"} hover:bg-[rgb(249, 245, 252)]`}
                            >
                                <td className='pl-[1rem] py-4 max-w-[120px]'>
                                    <div
                                        className={`text-sm font-medium line-clamp-2 break-words overflow-hidden
                                    ...
                                    <div className='text-sm text-gray-500'>
                                        ...
                                        coupon?.applicableType === "products" ? (
                                            ...
                                            >
                                                ...
                                                <span
                                                    className={`text-[11px] 
                                                        ${coupon.status === "active" ? "text-secondary" : "text-muted"}
                                            ...
                                            >
                                                ...
                                                <span className='text-[11px] text-secondary hover:underline hover:font-medium 
                                        ...
                                        <p className='relative text-[11px]'>
                                            {" "}
                                            Customer Specific - {`${coupon?.customerSpecific ? "Yes" : "No"}`}
                                            {coupon?.customerSpecific && (
                                                <span
                                                    className='text-[11px] text-secondary hover:underline hover:font-medium cursor-pointer'
                                                    onMouseEnter={() => setShowCustomersOf(coupon?.code)}
                                                    onMouseLeave={() => setShowCustomersOf("")}
                                                >
                                                    <span
                                                        className={`${coupon.status === "active" ? "text-secondary" : "text-muted"}`}>
                                                        &nbsp; [See]
                                                    </span>
                                ...
                                <td className='px-6 py-4 whitespace-nowrap'>
                                    ...
                                    <p className='text-[11px] text-muted'>
                                        Limit / Customer -{" "}
                                        {`${coupon?.usageLimitPerCustomer ? coupon.usageLimitPerCustomer : "Not available"}`}
                                    </p>
                                </td>
                                <td className='px-6 py-4 whitespace-nowrap'>
                                    <div
                                        className={`w-[2rem] text-sm text-gray-500 capitalize ${coupon.status === "active" 
                                            ? "text-muted" : "text-red-500"} `}>
                                        {coupon?.status}
                                    </div>
