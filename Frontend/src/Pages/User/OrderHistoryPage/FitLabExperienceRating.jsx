    ...
    return (
        <motion.div
            ...
            className='mt-6 ml-6 max-w-[35%] bg-gradient-to-r from-purple-50 to-purple-50 border-l-4 border-purple-400 r\
                rounded-lg rounded-tl-[8px] rounded-bl-[8px] p-5 md:p-7 shadow-lg'
        >
            <div className='flex items-start justify-between gap-3 md:gap-4 mb-4'>
                <div className='flex items-start gap-3 md:gap-4 flex-1'>
                    ...
                    <div>
                        <p className='text-base md:text-[16px] font-bold text-gray-900 text-secondary'>
                            How was your FitLab experience?
                        </p>
                        <p className='text-xs md:text-[13px] text-gray-600 '>
                            Help us improve your shopping journey. Rate each aspect:
                        </p>
                    </div>
                </div>
                <button
                    ...
                    {categories.map((cat, idx) => {
                        ...
                        return (
                            <motion.button
                                ...
                                className={`relative py-[10px] px-[5px] rounded-lg transition-all duration-200 focus:outline-none 
                                ...
                                <motion.div
                                    ...
                                    <Star
                                        className={`w-8 h-8 md:w-10 md:h-10 transition-colors ${
                                            rating[category] >= star || hoveredRating >= star
                                                ? "text-primaryDark fill-primaryDark"
                                                : "text-gray-300"
                                        }`}
                                    />
                                </motion.div>
            ...
            <motion.div
                initial={{ opacity: 0, y: 10 }}
                animate={{ opacity: 1, y: 0 }}
                className='flex gap-2 justify-between md:gap-3'>
                {Object.values(rating).every((value) => value) && (
                    <button
                        onClick={handleSubmit}
                        className='flex-1 bg-gradient-to-r from-purple-500 to-purple-600 hover:from-purple-600 hover:to-purple-700
                        text-white text-[15px] font-semibold py-[8px] px-[10px] max-w-[45%] rounded-[7px] transition-all 
                        duration-200 focus:outline-none focus:ring-2 focus:ring-purple-400 focus:ring-offset-2 shadow-md
                        hover:shadow-lg'
                    >
                        Submit
                    </button>
                ...
                {Object.values(rating).some((value) => value) && (
                    <button
                        ...
                        className={`flex-1 bg-gray-200 ${rating[category] && "hover:bg-gray-300"} text-gray-800 font-semibold
                            rounded-lg transition-colors duration-200 focus:outline-none focus:ring-2 disabled:cursor-not-allowed
                            focus:ring-gray-400 focus:ring-offset-2 py-[8px] px-[10px] 
                            ${!Object.values(rating).every((value) => value) ? "w-full" : "max-w-[45%] "}`}
