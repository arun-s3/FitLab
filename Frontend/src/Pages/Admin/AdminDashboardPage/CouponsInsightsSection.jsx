...
export default function CouponsOffersInsightsSection() {
    ...
    useEffect(() => {
        ...
        const orderedStats = couponStats.sort((a, b) => priorityOrder.indexOf(a.name) - priorityOrder.indexOf(b.name))
    ...
    return (
        ...
        >
            <h2 className='text-xl text-secondary font-bold flex items-center gap-[10px]'>
                <span className={`w-fit whitespace-nowrap ${!showOperationsAnalytics.coupons && "text-muted"}`}>
                    {" "}
                    Coupon Usage{" "}
                </span>
                <div
                    ...
                    onClick={() =>
                        togglerEnabled &&
                        setShowOperationsAnalytics((status) => ({ ...status, coupons: !status.coupons }))
                    }>
                    <hr
                    ...
                    >
                        ...
                        <motion.div
                            ...
                            className={`bg-white dark:bg-gray-800 p-6 rounded-lg shadow-sm border
                                ${dashboardQuery.trim() && !"Coupon Redemptions".toLowerCase().trim().includes(dashboardQuery.trim()) 
                                    ? "hidden" 
                                    : "inline-block"
                                }`}
                        >
                            ...
                            <div className={`${status.couponRedemptionsDatas !== "error" ? "h-80" : "h-40"} relative`}>
                                {status.couponRedemptionsDatas !== "loading" ? (
                                    <ResponsiveContainer width='100%' height='100%'>
                                        ...
                                        {status.couponRedemptionsDatas !== "error" &&
                                            couponRedemptionsDatas &&
                                            (couponRedemptionsDatas.length === 0 ||
                                                couponRedemptionsDatas.every((data) => data.redemptions === 0)) && (
                                                <p
                        ...
                        <motion.div
                            ...
                            className={`bg-white dark:bg-gray-800 p-6 rounded-lg shadow-sm border
                                ${dashboardQuery.trim() && 
                                    !"Discount Impact on Sales".toLowerCase().trim().includes(dashboardQuery.trim()) 
                                        ? "hidden" 
                                        : "inline-block"
                                }`}>
                            ...
                            <div className={`${status.discountImpactDatas !== "error" ? "h-80" : "h-40"} relative`}>
                                {status.discountImpactDatas !== "loading" ? (
                                    <ResponsiveContainer width='100%' height='100%'>
                                        ...
                                        {status.discountImpactDatas !== "error" &&
                                            discountImpactDatas &&
                                            (discountImpactDatas.length === 0 ||
                                                discountImpactDatas.every(
                                                    (data) => data.withDiscount === 0 && data.withoutDiscount === 0,
                                                )) && (
                                                <p
                                                    className='absolute -top-[10.5rem] sm:top-0 w-full h-full flex items-center 
                                    justify-center text-muted  text-[13px] sm:text-[16px] font-medium tracking-[0.3px]'>
