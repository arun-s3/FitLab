            ...
            <div className='max-w-7xl mx-auto'>
                <div className='flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8'>
                    <div className='mb-8 sm:mb-0'>
                        {subtitle && <p className='text-secondary font-semibold text-sm tracking-wide'> {subtitle} </p>}
                        {title && (
                            <h2
                                className={`${size === "small" ? "text-[28px] sm:text-[30px]" : "text-[30px] sm:text-[40px]"} 
                                    font-bold text-gray-900 ${titleStyle && titleStyle}`}>
                                {" "}
                                {title}{" "}
                            </h2>
                        )}
                                ...
                                {visibleProducts.map((product, index) => (
                                    ...
                                    >
                                        <motion.div
                                            ...
                                            className={`relative ${size === "small" ? "h-[17rem]" : "h-96"} 
                                                rounded-3xl overflow-hidden shadow-lg group 
                                        ...
                                        >
                                            ...
                                            >
                                                <button
                                                    className='bg-primaryDark hover:bg-green-500 text-white font-semibold px-6 py-2 
                ...
                {error && (
                    <motion.div
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        className='mt-16 mx-auto flex justify-center items-center gap-[5px] w-[35%] min-w-[15rem] 
                            bg-red-50 border border-red-200 text-red-700 px-4 py-8 rounded-lg mb-8'
                    >
                            ...
                            <span className='flex items-center gap-[7px] text-[17px] text-[#686262] font-medium'>
                                ...
                                <RotateCcw
                                    className='w-[20px] h-[20px] text-muted p-1 rounded-full border 
                                        border-dropdownBorder cursor-pointer hover:text-black transition-all duration-150 ease-in'
                                    onClick={() => refetch()}
                                />
                            </span>
