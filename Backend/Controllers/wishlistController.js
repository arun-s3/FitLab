...
const addProductToList = async (req, res, next) => {
    try {
        ...
        if (listIndex !== -1) {
            ...
            wishlist.lists.push({
                name: targetListName,
                thumbnail: productExists.thumbnail.url,
                products: [{ product: productId, notes: productNote, productPriority }],
            })
        }
        await wishlist.save()
...
const getAllWishlistProducts = async (req, res, next) => {
    try {
        ...
        productsArray = productsArray.filter((item) => item.product && !item.product.variantOf)
            ...
            filteredProducts.sort((a, b) => {
                ...
                switch (sortKey) {
                    ...
                    case "alphabetical":
                        return (
                            sortOrder *
                            (productA.title?.toLowerCase() || "").localeCompare(productB.title?.toLowerCase() || "")
                        )
                    case "priority":
                        return sortOrder * ((a.productPriority || 2) - (b.productPriority || 2))
                    default:
                        return 0
                }
            })
...
const searchList = async (req, res, next) => {
    try {
        ...
        const matchedLists = wishlist.lists.filter((list) => list.name.toLowerCase().includes(find.toLowerCase()))
