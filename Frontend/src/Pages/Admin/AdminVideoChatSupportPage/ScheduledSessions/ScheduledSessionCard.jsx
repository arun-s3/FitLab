    ...
    useEffect(() => {
        if (Object.keys(session).length > 0) {
            if (session?.timeRemaining) {
                ...
                setIsUrgent(
                    Object.keys(timeInfoObj).length > 0 ? timeInfo.urgent && session.status === "upcoming" : false,
                )
            }
        }
                    ...
                    <div className='flex items-start justify-between mb-4'>
                        ...
                        {isUrgent && (
                            <motion.div
                                ...
                                className='flex items-center space-x-1 bg-orange-100 text-orange-600 px-2 py-1 
                    ...
                    {session.notes && (
                        <div className='bg-gray-50 rounded-lg p-3 mb-4'>
                            <p className='text-[13px] text-gray-700'>
                                ...
                                {session.notes.length > 60 ? session.notes.slice(0, 50) + "..." : session.notes}
                            </p>
                        </div>
                    )}
                </div>
                ...
                <div className='flex items-center gap-8 space-x-3 mt-4 lg:mt-0 lg:ml-6'>
                    ...
                    <div className='flex gap-[5px] space-x-2'>
                        <motion.button
                            ...
                            whileTap={{ scale: 0.95 }}
                            className='p-3 rounded-[9px] bg-blue-100 text-blue-600 hover:bg-blue-200 
                        ...
                        {session.timeRemainingMs && (
                            <motion.button
                                ...
                                className={`flex items-center space-x-2 px-4 py-3 rounded-[7px] font-semibold 
                                    transition-all duration-300 ${
                                    !session.isUserActive
                                        ? "bg-gray-200 text-gray-400 cursor-not-allowed"
                                        : disableCall
                                          ? "bg-gray-200 text-gray-400 cursor-not-allowed"
                                          : `bg-gradient-to-r ${isCalling 
                                                ? `from-red-400 to-red-500 hover:from-red-500 hover:to-red-600` 
                                                : "from-green-500 to-emerald-600 hover:from-green-500 hover:to-emerald-600"} 
                            ...
                            >
                                ...
                                <span className='hidden sm:inline text-[14px] tracking-[0.3px]'>
                                    {session.isUserActive && isCalling
                                        ? "End Call"
                                        : session.isUserActive
                                          ? "Start Call"
                                          : "User Offline"}
