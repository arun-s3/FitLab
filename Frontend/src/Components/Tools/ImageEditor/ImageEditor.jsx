...
export default function ImageEditor() {
    ...
    useEffect(() => {
        if (apply) {
            loadStartInterval.current = setInterval(() => {
                setLoadStart((num) => (num >= 90 ? 90 : num + 15))
            }, 250)
        }
    }, [apply])
    ...
    const applyEffects = ({ previewMode = false }) => {
        ...
        img.onload = async () => {
            ...
            await applyColorCorrections(canvas, ctx, {
                channelR,
                channelG,
                channelB,
                tempR,
    ...
    return (
        <section id='image-editor' className='h-screen w-full'>
            ...
            <header
                className='relative flex justify-between items-center h-[4rem] w-full pl-[3rem] rounded-bl-[60px]'
                style={headerBg}
            >
            ...
            <div className='fixed h-screen flex'>
                ...
                <main className='flex h-screen absolute left-[4rem] w-[90%]'>
                    {showControlPanel && (
                        <div className='basis-[20%] w-[20%] border-r-2 border-l-borderLight pt-[1rem] px-[2rem] control-panel'>
                            {showPanel.colorAdjuster && (
                                ...
                                <ColorAdjuster
                                    params={{ brightness, contrast, saturate }}
                                    setters={{ setBrightness, setContrast, setSaturate }}
                                />
                            ...
                            {showPanel.filters && (
                                ...
                                <Filters
                                    params={{ grayscale, blur, sepia, opacity }}
                                    setters={{ setGrayscale, setBlur, setSepia, setOpacity }}
                                />
                    ...
                    <div className='h-[85%] w-full content-panel'>
                        <figure className='h-[97%] w-full p-[2rem] flex justify-center items-center bg-grayLightMuted'>
                            {!showPanel.cropper ? (
                                ...
                                <>
                                    ...
                                    <button
                                        className='absolute bottom-[8.1rem] left-[17.4rem] px-[18px] py-1 text-white text-[15px] 
                                            font-medium tracking-[0.3px] bg-secondary rounded-[4px] hover:bg-purple-700 
                        ...
                        <div className='h-auto w-full border-t-2 border-l-borderLight px-[1.5rem] pt-[15px] py-[10px] 
                        ...
                        >
                            ...
                            <div className='flex items-center gap-[1.3rem]'>
                                ...
                                <SiteButtonSquare
                                    customStyle={{ width: "6rem", paddingBlock: "6px" }}
                                    clickHandler={() => applyEffects({ previewMode: false })}
                                >
