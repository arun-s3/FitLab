...
import { SiteSecondaryButtonSquare } from "../../UI/SiteButtons/SiteButtons"
...
export default function PriceSliderAndFilter({ priceGetter, priceSetter, firstSlide, mountingComponent }) {
    ...
    let firstRangeStart = useRef(0)
    let secondRangeStart = useRef(0)
    let firstRangeEnd = useRef(0)
    let secondRangeEnd = useRef(0)
    let firstRange = useRef(0)
    let secondRange = useRef(0)
    let currentLeft = useRef(0)
    let collisionCheck = useRef(false)
    const showRangeRef = useRef(null)

    let currentX = useRef(0)
    let leftValueNow = useRef(0) // For mouse events
    let checkDragging = useRef(false)
    let checkMouseDown = useRef(false) // For mouse events
    ...
    const setPriceUnit = (e) => {
        ...
        if (
            minPriceRef.current.getBoundingClientRect().right <= maxPriceRef.current.getBoundingClientRect().left ||
    ...
    return (
        ...
        >
            <p
                className={`${(mountingComponent == "AdminProductListPage" ? "text-[13px] " : "text-[15px] ") + 
                    "text-secondary text-center"}`
            }>
                {minPrice} - {maxPrice ? (maxPrice !== maxPriceRupee.current ? maxPrice : maxPrice + "+") : "-"}
            </p>
            <div
                id='pricerange-wrapper'
                className='relative mt-[1rem]'
                onDragOver={(e) => dragOverHandler(e)}
                onDrop={(e) => dropHandler(e)}
                ref={priceRangeWrapperRef}
            >
                <div
                    className='relative w-[15rem] h-[4px] bg-[#AFD0FF] rounded-[2px]'
                    onDragOver={(e) => dragOverHandler(e)}
                    onDrop={(e) => dropHandler(e)}
                >
                    {" "}
                    {/* ref={rangeRef} bg-[#cfb6ee]*/}
                    <div
                        draggable='true'
                        className='absolute left-0 top-[-8px] w-[20px] h-[20px] rounded-[10px] 
                           border-[2px] border-[#AFD0FF]'
                        onDragStart={(e) => dragStartHandler(e, firstRangeStart)}
                        onDrag={(e) => dragHandler(e, "firstHandler")}
                        onDragEnd={(e) => dragEndHandler(e, firstRangeStart, firstRangeEnd, firstRange)}
                        onClick={(e) => {
                            e.target.draggable = true
                            collisionCheck.current = false
                        }}
                    ...
                    <div
                        draggable='true'
                        className='absolute left-[9px] top-[-8px] w-[20px] h-[20px] rounded-[10px] 
                         border-[2px] border-[#AFD0FF]'
                        onDragStart={(e) => dragStartHandler(e, secondRangeStart)}
                        onDrag={(e) => dragHandler(e, "secondHandler")}
                        onDragEnd={(e) => dragEndHandler(e, secondRangeStart, secondRangeEnd, secondRange)}
                        onClick={(e) => (e.target.draggable = true)}
                        ref={maxPriceRef}
                    ></div>
            ...
            <div>
                <div
                    className='flex justify-between w-full mb-[5px] price-buttons mt-[25px]'
                    style={mountingComponent == "AdminProductListPage" ? { marginTop: "8px" } : {}}
                >
                    <div>
                        ...
                        <div className='flex items-center'>
                            <button
                                ...
                                style={mountingComponent == "AdminProductListPage" ? { height: "1.4rem" } : {}}
                            >
                                {" "}
                                +{" "}
                            </button>
                            <input
                                type='number'
                                defaultValue='0'
                                className='w-[70px] border-[#ced1d7] border-l-0 border-r-0
                                      p-[1px] h-[23px] placeholder:text-secondary placeholder:text-[14px]'
                                style={
                                    mountingComponent == "AdminProductListPage"
                            ...
                            <button
                                className='incdec-btn'
                                onClick={(e) => {
                                    setMinInputPrice((price) =>
                                        Number.parseInt(price) - 1 < 0 ? 0 : Number.parseInt(price) - 1,
                                    )
                                }}
                                style={mountingComponent == "AdminProductListPage" ? { height: "1.4rem" } : {}}
                            >
                    ...
                    <div>
                        ...
                        <div className='flex items-center'>
                            <button
                                className='incdec-btn'
                                onClick={(e) => {
                                    setMaxInputPrice((price) => Number.parseInt(price) + 1)
                                }}
                                style={mountingComponent == "AdminProductListPage" ? { height: "1.4rem" } : {}}
                            >
                                {" "}
                ...
                {mountingComponent == "AdminProductListPage" ? null : (
                    <div onClick={() => validateAndCompute()} className='mt-[15px] text-center'>
                        <SiteSecondaryButtonSquare
                            customStyle={{ width: "60%", paddingBlock: "5px", fontSize: "13px" }}
                        >
                            {" "}
                            Apply{" "}
                        </SiteSecondaryButtonSquare>
                ...
                <p
                    className='invisible w-full h-[10px] text-red-500 text-[10px] mt-[5px] text-center'
                    ref={priceErrorRef}></p>
