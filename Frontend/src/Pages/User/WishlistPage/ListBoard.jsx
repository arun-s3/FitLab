    ...
    return (
        <div className='max-md:relative'>
                            ...
                            [...wishlist.lists]
                                ...
                                .map((list) =>
                                    isListCardLoading?.[list._id] ? (
                                        <div
                                            key={list.name}
                                            className='w-full h-[240.6px] px-[12px] py-[10px] border border-dropdownBorder 
                                                rounded-[5px] shadow-lg cursor-default skeleton-loader'
                                        />
                                    ) : (
                                        <motion.div
                                            key={list.name}
                                            ...
                                            className={`relative w-full px-[12px] py-[10px] min-w-[11rem]
                                                ${currentList === list.name ? "bg-whitesmoke shadow-lg" : "bg-white shadow-sm"} 
                                                flex flex-row md:flex-col gap-[10px] justify-between border border-dropdownBorder 
                                                rounded-[5px] cursor-pointer hover:bg-whitesmoke hover:shadow-md`}
                                            id='list'
                                        >
                                            <SquarePen
                                                className='absolute left-[7px] right-auto md:right-[7px] md:left-auto top-[7px] 
                                                    w-4 h-4 text-[#aa65db] transition duration-300 hover:text-secondary 
                                                    hover:scale-105 hover:rotate-[360deg] cursor-pointer'
                                                onClick={() => onUpdateList(list)}
                                            />
                                            <div
                                                className='ml-[7px] md:ml-0 flex flex-col justify-between gap-[10px] self-center'
                                                onClick={() =>
                                                    onNewCurrenList((prev) => (prev === list.name ? "" : list.name))
                                                }
                                            >
                                                <div>
                                                    <figure
                                                        className={` ${
                                                            !list?.thumbnail && "flex justify-center items-center"
                                            ...
                                            <div className='w-auto md:w-full flex flex-col md:flex-row justify-between controls'>
                                                <Trash
                                                    className='w-[15px] h-[15px] text-muted hover:scale-110 hover:text-red-500 
            ...
            >
                ...
                <span className='text-[14px] text-muted font-[500] tracking-[0.5px] capitalize'>Add New List</span>
            </motion.div>
        </div>
    )
}
