  ...
  useEffect(() => {
    const fetchAllStats = async ()=> {   
      try {   
        ...
        const response = await axios.get(`${baseApiUrl}/fitness/tracker/stats/${dataKey}`, { withCredentials: true })
            ...
            <ResponsiveContainer width="100%" height="100%">
              {isPieChart && data.length > 0 ? (
                <PieChart>
                  ...
                  >
                    {data &&
                      data.map((entry, index) => <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} fontSize={13}/>)}
                  </Pie>
                ...
                <AreaChart data={data} margin={{ top: 5, right: 10, left: -20, bottom: 5 }}>
                  ...
                  />
                  <Area type="monotone" dataKey="value" stroke={config.color} fillOpacity={1} fill="url(#colorArea)" fontSize={13}/>
                </AreaChart>
