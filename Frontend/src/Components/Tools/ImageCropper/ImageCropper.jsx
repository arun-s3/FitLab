...
} from "../../UI/SiteButtons/SiteButtons"
...
const ImageCropper = ({
    ...
    const [currentImageIndex, setCurrentImageIndex] = useState(() =>
        imageCropperDefaultIndex ? imageCropperDefaultIndex : 0,
    )
        ...
        <section id='imageCropper'>
            {modalIsOpen && (
                <main
                    ...
                    className={`fixed flex flex-col justify-center items-center gap-[16px] inset-0 
                        ${positionFromTop ? positionFromTop : "top-[3rem]"} ${bgBlur && "backdrop-blur-[5px]"} z-[5]`}>
                    <div
                        id='image-cropper-content'
                        className={`w-[400px]  ${images.length > 1 ? "max-w-[90%]" : "!max-w-[68%]"} text-center 
                    ...
                    >
                        {images.length > 1 && (
                            <div className='flex items-center gap-[30px] flex-wrap my-[10px]' id='image-section'>
                                <i
                                    className='mr-[-20px] px-[2px] py-[4px] rounded-[4px] bg-primary border-[2px] 
                                        border-[#e6c5fd] cursor-pointer'
                                ...
                                {images.map((image, index) => (
                                    <div
                                        ...
                                        className={`relative flex flex-col cursor-pointer rounded-[5px] 
                                            ${images.length > 0 && index >= startIndex && index < endIndex ? "" : "hidden"}
                                            ${buildOutline && buildOutline === image.name 
                                                ? "outline outline-[1.7px] outline-secondary outline-offset-[5px]" : ""}
                                    ...
                                    >
                                        <figure key={image.name} className='relative w-[75px] h-[75px] rounded-[5px]'>
                                            ...
                                            />
                                            <span className='absolute top-0 right-[-22px] h-full flex flex-col justify-between
                                                text-secondary cursor-pointer '
                                            >
                                                <span className=' rounded-[4px] text-secondary'>
                                                    <IoCloseSharp
                                                        onClick={(e) => imageCloseHandler(image.url, index)}
                                                    />
                                                </span>
                                            </span>
                                            {!image?.isCropped && (
                                                <span
                                                    className='absolute bottom-[7px] left-[25%] px-[5px] py-[2px] rounded-[4px] bg-primary text-[9px]
                                ...
                                <i
                                    className='ml-[-15px] px-[2px] py-[4px] rounded-[4px] bg-primary border-[2px] border-[#e6c5fd] cursor-pointer'
                        ...
                        {images[currentImageIndex] && (
                            <>
                                <div className='relative w-full h-[300px] bg-[#333] border-[2px] border-[#ddd] rounded-[6px]'>
                                    ...
                                    <div
                                        className={`w-full absolute ${images.length > 1 ? "top-[22rem]" : "top-[19rem]"} left-[5%] flex flex-col gap-[1rem] mt-[20px]`}
                                        style={controllerStyle ? controllerStyle : {}}
                                    >
                                        <div className='flex items-center gap-[1rem]'>
                                            <label
                                                htmlFor='zoom-range'
                                                className='text-[13px] text-secondary font-[500] tracking-[0.7px]'
                                            ...
                                            <input
                                                ...
                                                className={` ${images.length > 1 ? "w-[200px]" : "w-[250px]"} h-[3px]`}
                                            />
                                        </div>
                                        <div className={`flex gap-[10px] ${images.length === 1 && "justify-center"}`}>
                                            {images.length > 1 && (
                                                <SiteButtonSquare
                                                    clickHandler={() => skipCrop(currentImageIndex)}
                                                    tailwindClasses='!text-[13px]'
                                                    customStyle={{
                                                        paddingBlock: "7px",
                                                        borderRadius: "7px",
                                                        fontSize: "13px",
                                        ...
                                        <p className='mt-[-10px] ml-[-50px] h-[5px] text-[10px] text-red-500 
                        ...
                        {images.length > 1 && currentImageIndex < images.length && currentImageIndex >= 0 && (
                            <>
                                <div className='flex justify-between items-center mt-[10px]'>
                                    <SitePrimaryButtonWithShadow
                                        tailwindClasses='text-secondary flex flex-row-reverse items-center gap-[5px] 
                                            bg-primaryDark hover:bg-green-500'
                                        customStyle={{}}
                                        clickHandler={() =>
                                    ...
                                    <SitePrimaryButtonWithShadow
                                        tailwindClasses='text-secondary flex items-center gap-[5px] bg-primaryDark hover:bg-green-500'
                                        clickHandler={() => {
                                            if (currentImageIndex === images.length - 1)
                                                setImageCropperError(
                                                    "Make sure you crop every image later before submitting",
                                                )
                                            setCurrentImageIndex(
                                ...
                                <div className='mt-[5px] flex justify-between'>
                                    <p className='text-[9px] text-secondary font-[400]'>
                                        All images must be cropped before submission, but cropping can be skipped and
                                        completed later if necessary
                                        <SitePrimaryMinimalButtonWithShadow
                                            tailwindClasses='ml-[10px] text-[10px] px-[7px]'
