...
export default function OfferList({ offers, onEdit, onDelete, onSort, onDeactivate }) {
    ...
    const tableHeaders = [
        { value: "Offer", icon: true, sortBy: "name" },
        { value: "Target Users", icon: false },
        { value: "Discount", icon: false },
        { value: "Recurring Offer", icon: false },
        { value: "Start Date", icon: true, sortBy: "startDate" },
        { value: "Expiry", icon: true, sortBy: "endDate" },
        { value: "Redemption Count", icon: true, sortBy: "redemptionCount" },
        { value: "Status", icon: false },
        { value: "Actions", icon: false },
    ]
                ...
                <thead className='bg-inputBorderLow'>
                    <tr>
                        {tableHeaders.map((header) => (
                            <th
                                key={header.value}
                                className='p-3 max-w-[270px] text-left text-[12px] font-medium text-gray-500 uppercase tracking-wider
                                    cursor-pointer'
                                onClick={() => {
                            ...
                            >
                                <div className='flex items-center min-w-0'>
                                    ...
                                    {header.icon && (
                                        <ArrowUpDown className='h-4 w-4 ml-1 hover:text-purple-700 hover:scale-110 transition 
                                            duration-100' />
                                    )}
                                </div>
                            </th>
                ...
                <tbody className='bg-white divide-y divide-gray-200'>
                    {offers && offers.length > 0 ? (
                        offers.map((offer, index) => (
                            <tr
                                key={offer?._id}
                                className={`${index % 2 == 0 ? "bg-transparent" : "bg-[#eee]"} relative hover:bg-[rgb(249, 245, 252)]`}
                            >
                                <td className='pl-4 py-4 max-w-[280px] border-r border-dashed border-[#A399A880]'>
                                    <div className='min-w-0 mb-4 flex items-center gap-[10px]'>
                                        ...
                                        <span
                                            className={`text-sm font-medium capitalize line-clamp-2 break-words overflow-hidden block
                                    ...
                                    {offer.offerBanner ? (
                                        <figure className='relative group inline-block w-[150px] h-[50px] p-[2px] rounded-[4px]'>
                                            <img
                                                ...
                                                alt={offer.offerBanner.name}
                                                className='w-[150px] h-[50px] object-cover border border-primaryDark rounded-[4px]'
                                            />
                                    ...
                                    <div className='mt-[5px]'>
                                        <div className='flex items-start gap-[10px] min-w-0'>
                                            ...
                                            <div>
                                                <p className='text-[12.5px] text-gray-500 font-[500]'>
                                                    {" "}
                                                    Applicable to :{" "}
                                                </p>
                                                <div className='text-sm text-gray-500'>
                                                    {offer?.applicableType === "allProducts" ||
                                                    offer?.applicableType === "products" ? (
                                                        <span
                                                            className='relative'
                                                            onMouseEnter={() => setShowItemsOf(offer?.name)}
                                                            onMouseLeave={() => setShowItemsOf("")}
                                                        >
                                                            <span className='text-[12.5px]'> Products - </span>
                                                            <span
                                                                className={`text-[11px] 
                                                                    ${offer.status === "active" ? "text-secondary" : "text-muted"} 
                                                                    hover:underline hover:font-medium transition duration-300 
                                                                    cursor-pointer`}
                                                            >
                                                                {offer?.applicableType === "allProducts"
                                                                    ? "All"
                                                            ...
                                                            {showItemsOf === offer.name &&
                                                                (offer?.applicableType === "allProducts" ? (
                                                                    <div
                                                                        className={`absolute bottom-[100%] left-[50%] py-[10px] px-[10px] 
                                                                            bg-white border border-dropdownBorder
                                                                            rounded-[4px] z-[10] capitalize text-muted opacity-100 
                                                        ...
                                                        >
                                                            ...
                                                            <span
                                                                className={`text-[11px] 
                                                                    ${offer.status === "active" ? "text-secondary" : "text-muted"} 
                                                                    hover:underline hover:font-medium transition 
                                ...
                                <td className='pl-[1rem] py-4 whitespace-nowrap'>
                                    <div className='text-sm capitalize text-gray-500'>{offer.targetUserGroup}</div>
                                </td>

                                <td className='px-6 py-4 whitespace-nowrap'>
                                    <div
                                        className={`text-sm capitalize ${offer.status === "active" ? "text-secondary" : "text-muted"} `}>
                                ...
                                <td className='px-6 py-4 whitespace-nowrap'>
                                    ...
                                    {offer?.usedCount ? (
                                        <p className='text-[11px] text-muted'>Used Count - {offer.usedCount}</p>
                                    ) : null}
                                    {offer?.completedCount >= 0 && offer?.impressionCount >= 0 ? (
                                        <p
                                            className={`text-[11px] ${offer.status === "active" ? "text-secondary" : "text-muted"}`}
                                        >
                                            Conversion Rate -{" "}
                                ...
                                <td className='px-6 py-4 whitespace-nowrap relative'>
                                    <div
                                        className={`w-[2rem] text-sm text-gray-500 capitalize 
                                    ...
                                    {offer?.lastUsedAt ? (
                                        <div
                                            className={` absolute !text-[11px] text-gray-500 capitalize 
                                                ${offer.status === "active" ? "text-green-500" : "text-red-500"}`}
                                        >
