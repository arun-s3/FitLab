...
export default function Pagination({ productCounts, currentPage, currentPageChanger, limiter }) {
    ...
    return (
        <div className='mt-[3rem] flex gap-[10px] items-center' id='pagination-bar'>
            ...
            <span
                onClick={() => currentPageChanger(currentPaginationPage)}
                style={totalPages == 1 && { background: "white", borderColor: "#D5D2D2" }}
            >
                {" "}
                {/*1*/}
            ...
            {totalPages > 1 && (
                <span onClick={() => currentPageChanger(currentPaginationPage + 1)}>
                    {" "}
                    {/*2*/}
            ...
            {totalPages > 4 && (
                <span onClick={() => currentPageChanger(totalPages)}>
                    {" "}
                    {/*LastPage*/}
                    {totalPages}
                </span>
