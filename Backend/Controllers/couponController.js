...
const getEligibleCoupons = async (req, res, next) => {
    try {
        ...
        const { page = 1, limit = 6, sort = -1, sortBy = "createdAt", searchData } = queryOptions
        ...
        coupons = coupons.filter((coupon) => {
            ...
            if (userId) {
                ...
                if (
                    ...
                    userUsage &&
                    userUsage.count >= coupon.usageLimitPerCustomer
                )
                    return false
            ...
            for (const item of cart.products) {
                ...
                const isApplicable =
                    ...
                    (coupon.applicableType === "categories" &&
                        product.category.some((catId) =>
                            coupon.applicableCategories.map((id) => id.toString()).includes(catId.toString()),
                        ))
    ...
    try {
        ...
        const checkCouponValidity = async (coupon) => {
            ...
            if (
                ...
                !coupon.assignedCustomers.some((user) => user.equals(new mongoose.Types.ObjectId(userId)))
            ) {
                return next(
