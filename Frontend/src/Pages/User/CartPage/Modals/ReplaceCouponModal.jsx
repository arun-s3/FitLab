            ...
            try {
                ...
                if (response.status === 200) {
                    ...
                    if (currentCoupon === response.data.winnerCoupon.code)
                        setWinnerCouponDiscount({ currentCoupon: true, newCoupon: false })
                        ...
                        <div className='p-6' ref={modalRef}>
                            <p className='text-gray-600 text-[14px] mb-4'>
                                {!couponError && winnerCouponDiscount.newCoupon ? (
                                    <div>
                                        <span>
                                            You already have a coupon applied to your order. Do you want to replace it
                                            with the new coupon?
                                        </span>
                                    ...
                                    <span>
                                        <span>
                                            {" "}
                                            You already have a coupon with a better discount value providing you a
                                            discount
                                        </span>
                                        {cart?.couponUsed?.discountType !== "buyOneGetOne" && <span> of </span>}
                                        <span className='text-red-500 font-[480]'>
                                            {cart?.couponUsed?.discountType !== "buyOneGetOne" &&
                                                `${`( ${
                                                    cart?.couponUsed?.discountType === "percentage"
                                                        ? cart?.couponUsed?.discountValue + "%" + " Off"
                                                        : cart?.couponUsed?.discountType === "fixed"
                                                          ? "₹" + cart?.couponUsed?.discountValue + " Off"
                                                          : null
                                                        }
                                                    )
                                        ...
                                        <p className='mt-[10px]'>
                                            {" "}
                                            Do you want to replace it with the new coupon with a lesser discount
                                            value?{" "}
                                        </p>
                                    </span>
                                ) : null}
                            ...
                            <div className='bg-gray-50 rounded-lg px-4 mb-4'>
                                <div className='mb-2'>
                                    ...
                                    <span className='text-[15px] text-green-500 ml-[5px] uppercase'>
                                        {" "}
                                        {currentCoupon}{" "}
                                    </span>
                                </div>
                            ...
                            <div
                                className='flex xxs-sm:flex-row flex-col justify-center xxs-sm:justify-center items-center
                                    gap-4 xxs-sm:gap-8 space-x-3'>
                                <motion.button
                                    onClick={putOldCoupon}
                                    className='w-full xxs-sm:w-auto px-4 py-2 border border-gray-300 rounded-md 
                                ...
                                <motion.button
                                    ...
                                    className='w-full xxs-sm:w-auto !ml-0 xxs-sm:ml-auto px-4 py-2 border border-transparent rounded-md shadow-sm 
                                        text-sm font-medium text-white bg-secondary hover:bg-purple-700 
