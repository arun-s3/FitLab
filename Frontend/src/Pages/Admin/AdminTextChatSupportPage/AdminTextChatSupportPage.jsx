...
import {Users, Send, Search, Clock, User, Headphones, Circle, MessageSquare, History} from "lucide-react"
...
export default function AdminTextChatSupportPage() {
    ...
    setPageBgUrl(
        `linear-gradient(to right,rgba(255,255,255,0.93),rgba(255,255,255,0.93)), url('/Images/admin-bg8.png')`,
    )
    ...
    const handleUserSelect = (user) => {
        ...
        const notificationForUserExists =
            notifications.length > 0 && notifications.some((statusObj) => statusObj.sender === user.username)
    ...
    useEffect(() => {
        ...
        setOfflineUsers((prev) =>
            prev.filter(
                (offlineUser) => !activeUsers.some((activeUser) => activeUser.username === offlineUser.username),
            ),
        )
    }, [activeUsers])
    ...
    useEffect(() => {
        ...
        const matchedUser = activeUsers.find((user) => user.username === selectedUser.username)
    ...
    const getAdminChatHistory = async () => {
        ...
        try {
            const response = await apiClient.get(`/chat/history?page=${currentHistoryPage}&limit=${limit}`, {
    ...
    return (
        <section id='AdminTextChatSupportPage'>
            ...
            <header>
                <AdminTitleSection
                    heading='Text Chat Support'
                    subHeading='Manage and respond to customer inquiries in real-time through text chat for user satisfaction.'
                />
            </header>
            ...
            <main className='mr-4 p-4 border border-primaryDark rounded-[7px]'>
                <div className='h-screen bg-gray-100 flex'>
                    <div
                        className={`${guestCount > 0 ? "w-[22rem]" : "w-80"} p-[7px] bg-white border border-dropdownBorder 
                            rounded-[5px] flex flex-col`}
                    >
                        <div className='p-4 border-b border-gray-200 bg-purple-500 text-white rounded-[3px]'>
                            {" "}
                            ...
                            <div className='relative'>
                                <Search
                                    className='absolute left-3 top-1/2 transform -translate-y-1/2 text-secondaryLight2'
                                    size={16}
                                />
                                <input
                                    type='text'
                                    ...
                                    className='w-full h-8 pl-10 pr-4 py-2 bg-white text-secondary text-[13px] 
                                        placeholder-secondaryLight2 placeholder:text-[12px] rounded-lg border border-dropdownBorder 
                                        focus:outline-none focus:ring-2 focus:ring-secondary'
                                />
                                {usernameList && usernameList?.searched && openDropdowns.searchResultDropdown && (
                                    <motion.div
                                        className='absolute w-full py-[7px] px-[10px] bg-whitesmoke flex flex-col gap-[10px] border
                                    ...
                                    >
                                        {usernameList.list.length > 0 ? (
                                            usernameList.list.map((username) => (
                                                <span
                                                    key={username}
                                                    className='w-full px-[5px] text-[12px] text-muted flex items-center 
                                                        gap-[5px] hover:bg-primaryDark hover:text-secondary rounded-[4px] 
                                                        cursor-pointer'
                                                    onClick={() => selectSearchedUser(username)}
                                                >
                                                    {(() => {
                                                        ...
                                                        return (
                                                            <motion.div
                                                                className={`w-[5px] h-[5px] rounded-full 
                                                                    ${isOnline ? "bg-green-500" : "bg-red-500 scale-110"}`}
                                                                animate={
                                                                    isOnline && { scale: [1, 1.2], opacity: [0.5, 1] }
                                                                }
                                                                transition={
                                                                    isOnline && {
                                                                        duration: 1.5,
                                            ...
                                            <p className='h-[2rem] flex justify-center items-center text-muted 
                        ...
                        <div className='p-4 border-b border-gray-200 bg-gray-50'>
                            <div
                                className={`${guestCount > 0 ? "flex items-center justify-between gap-4" : "grid grid-cols-2 gap-4"}`}>
                                ...
                                <div className='text-center'>
                                    <div className='text-2xl font-bold text-green-600'>
                                        {
                                            activeUsers.filter(
                                                (user) => user.isOnline && !user.username.startsWith("guest"),
                                            ).length
                                        }
                                    </div>
                        ...
                        <div className='flex-1 overflow-y-auto'>
                                        ...
                                        >
                                            <div className='flex items-center justify-between'>
                                                <div className='w-full flex items-center space-x-3'>
                                                    <div className='relative'>
                                                        <div className='w-10 h-10 bg-gray-300 rounded-full flex items-center 
                                                            justify-center'
                                                        >
                                                    ...
                                                    <div className='flex-1 min-w-0 w-full'>
                                                        <div className='font-medium text-gray-900 truncate flex 
                                                        ...
                                                        >
                                                            ...
                                                            {notifications &&
                                                                ...
                                                                notifications.some(
                                                                    (status) => status.sender === user.username,
                                                                ) &&
                                                                notifications.find(
                                                                    (status) => status.sender === user.username,
                                                                ).msgCount > 0 && (
                            ...
                            {hasMoreUsersChats && (
                                <div
                                    className='w-full py-[10px] flex items-center justify-center gap-[10px] bg-gray-200 text-muted 
                                ...
                                >
                                    ...
                                    <span className='text-[14px] tracking-[0.3px] text-inherit font-inherit'>
                                        {" "}
                                        Show Chat History{" "}
                                    </span>
                                </div>
                                ...
                                >
                                    <div className='flex space-x-3'>
                                        <input
                                            ...
                                            placeholder={` ${
                                                textOrReply === "Text"
                                                    ? `Type a message for ${selectedUser.username}...`
                                                    : `Reply to ${selectedUser.username}...`
                                            } `}
                                            className='flex-1 border border-gray-300 rounded-lg px-4 py-2 text-[13px] 
