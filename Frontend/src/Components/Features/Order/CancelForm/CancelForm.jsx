...
export default function CancelForm({
    ...
    const commonReasonTitles = [
        ...
        "Product(s) price too high",
        "Poor product reviews discovered",
        "Need to change payment method",
        "Delivery charge too high",
        "Incorrect product ordered",
        "Found somewhere else cheaper",
        ...
        <div id='CancelForm' className='flex flex-col items-center'>
            <h2 className='mt-[2rem] flex items-center gap-[1rem]'>
                ...
                {!canceledByAdmin && !returnAndRefund && (
                    <span
                        className='relative px-[15px] py-[5px] w-[17rem] flex items-center justify-between border
                            border-dropdownBorder rounded-[6px] cursor-pointer'
                        ref={dropdownBoxRef}
                        id='dropdownBox'
                        onClick={() =>
                            setOpenSelectReasons((selectReason) => ({ ...selectReason, status: !selectReason.status }))
                        }
                    >
                        ...
                        {openSelectReasons.status && (
                            <ul
                                className='absolute top-[115%] left-0 w-[17rem] px-[25px] py-[15px] h-fit bg-white text-[13px]
                            ...
                            >
                                {commonReasonTitles.map((reasonTitle, index) => (
                                    <li key={index} onClick={(e) => cancelReasonHandler(e, { title: true })}>
                                        {" "}
                                        {reasonTitle}{" "}
                                    </li>
                                ))}
            ...
            <textarea
                ...
                cols='50'
                maxLength='500'
                placeholder='Please write your explanation (optional)'
                className='mt-[1rem] mb-[1.5rem] resize-none h-[7rem] text-[13px] placeholder:text-[10px] border
                   border-mutedDashedSeperation rounded-[5px] focus:ring-secondary focus:ring-[1.5px]
                      focus:outline-none focus-within:border-none focus-within:outline-none'
                value={openSelectReasons.reason}
                onChange={(e) =>
                    !canceledByAdmin
                        ? cancelReasonHandler(e, { content: true })
                        : cancelReasonHandler(e, { content: true, admin: true })
                }
            />

            <div className='flex items-center justify-center gap-[1rem]'>
                <SiteSecondaryFillImpButton
                    className='py-[5px] px-[10px] w-[6rem] capitalize'
                    clickHandler={() => submitReason(formFor)}
                    customStyle={{ marginTop: "0", paddingBlock: "5px", paddingInline: "25px", width: "fit-content" }}
                >
                    Submit
                </SiteSecondaryFillImpButton>
                <SiteSecondaryFillImpButton
                    className='py-[5px] px-[40px] w-[6rem]'
