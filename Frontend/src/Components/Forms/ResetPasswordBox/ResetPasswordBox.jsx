...
import { CustomHashLoader } from "../../UI/Loader/Loader"
...
export default function ResetPasswordBox({ setOpenSecurityMenu, admin }) {
    ...
    useEffect(() => {
        if (error.value && error.value.toLowerCase().includes("current")) {
            setTimeout(() => setError((error) => ({ ...error, current: false, value: "" })), 6500)
        }
    }, [error])
    ...
    const checkError = (e, type) => {
        ...
        if (value !== "" && !passwordPattern.test(value)) {
            setError((error) => ({
                ...error,
                [type]: true,
                value: "The password doesn't follow the given mandatory conditions!",
            }))
        } else {
            setError((error) => ({ ...error, [type]: false, value: "" }))
            return
        }
    }
    ...
    const focusHandler = () => {
        if (error.value && error.value.toLowerCase().includes("current")) {
            setError((error) => ({ ...error, current: false, value: "" }))
        }
    }
                ...
                <form onSubmit={(e) => submitPassword(e)} className='flex flex-col gap-[1.5rem] basis-[40%]'>
                    <div className='relative flex flex-col gap-[7px]'>
                        <input
                            ...
                            className={`peer w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-secondary 
                                focus:outline-none ${error.value.toLocaleLowerCase().includes("current")
                            ...
                            className={`absolute left-4 text-gray-500 text-[15px] transition-all duration-200  
                                ${
                                    passwords.current.length > 0
                                        ? "-top-3"
                                        : `peer-placeholder-shown:top-3 peer-focus:-top-3 peer-placeholder-shown:text-base 
                    ...
                    <div className='relative'>
                        <input
                            ...
                            className={`peer w-full px-4 py-3 placeholder:text-[15px] rounded-lg border ${
                                passwords.confirm &&
                                (passwordsMatch
                                    ? "border-green-500 focus:ring-green-200"
                                    : "border-red-500 focus:ring-red-200")
                        ...
                        <button
                            type='button'
                            className='absolute right-3 top-[43%] transform -translate-y-1/2 text-gray-400 hover:text-gray-600'
                    ...
                    <div>
                        {!admin && (
                            <Link
                                to='/forgot-password'
                                className='ml-[3xp] text-[12px] text-secondary tracking-[0.2x] hover:underline cursor-pointer'>
                                Forgot Password?
                            </Link>
                        )}
                        <SiteSecondaryFillImpButton
                            className='mt-[5xp] disabled:opacity-80 disabled:cursor-not-allowed'
                    ...
                    {!passwordsMatch && passwords.confirm && (
                        <p className='mt-[-22px] w-full h-[15px] text-center text-red-500 text-[12px] font-[500] 
                    ...
                    {requirementSubtitles.map((subtitle, index) => (
                        <li
                            ...
                            className={`flex items-center gap-2 
                                ${subtitle.requirement ? "text-green-500 line-through" : "text-gray-600"}`}
                        >
