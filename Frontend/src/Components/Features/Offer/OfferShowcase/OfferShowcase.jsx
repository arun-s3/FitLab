...
import { getAllOffers, increaseOfferImpression } from "../../../../Slices/offerSlice"
...
export default function OfferShowcase({ sectionStyle, containerStyle, headlineStyle }) {
    ...
    useEffect(() => {
        if (offers && offers.length > 0) {
            ...
            const thumbnail =
                offers[0].applicableType === "products"
    ...
    return (
        <section
            className={`w-full py-12 md:py-16 px-4 md:px-6 bg-gradient-to-b from-slate-50 to-white
                ${sectionStyle && sectionStyle}`}
        >
            <div className={`max-w-6xl mx-auto ${containerStyle && containerStyle}`}>
                <div className='mb-4 md:mb-6'>
                    <h2
                        className={`text-[28px] md:text-[34px] font-bold text-slate-900 text-balance 
                    ...
                    <div className='overflow-hidden rounded-xl bg-white dark:bg-slate-900 shadow-lg dark:shadow-2xl'>
                        <div className='relative h-64 md:h-96 lg:h-80'>
                            <AnimatePresence mode='wait'>
                                ...
                                >
                                    <div className='h-full flex flex-col md:flex-row items-center justify-between p-6 
                                        md:p-10 bg-gradient-to-r from-purple-50 to-indigo-50'
                                    >
                                        {currentOffer && (
                                            <div className='flex-1 flex flex-col justify-center mb-6 md:mb-0'>
                                                <motion.h3
                                                    ...
                                                    className='text-[20px] md:text-[30px] capitalize font-bold text-slate-900 mb-2 
                                                ...
                                                <motion.div
                                                    ...
                                                    <span
                                                        className='inline-block text-[17px] bg-primaryDark text-white 
                                                            px-4 py-[6px] rounded-lg font-bold text-lg md:text-xl'
                                                    >
                                                        {getDiscountLabel(currentOffer)}
                                                    </span>
                                                    {currentOffer.minimumOrderValue > 0 && (
                                                        <span
                                                            className='inline-block text-[14px] text-secondary bg-slate-200
                                                                font-medium px-3 py-2 rounded-lg text-xs md:text-sm'
                                                        >
                                                ...
                                                >
                                                    ...
                                                    {currentOffer.applicableType === "products" &&
                                                        currentOffer.applicableProducts?.length > 0 && (
                                                            <button
                                                                onClick={() => openProductsModal(currentOffer)}
                                                                className='bg-purple-500 text-white px-4 py-2 rounded-lg 
                                                                    text-sm font-medium hover:bg-purple-600 transition-colors'
                                                            >
                                                        ...
                                                        currentOffer.applicableCategories?.length > 0 && (
                                                            <button
                                                                ...
                                                                className='bg-purple-500 text-white px-4 py-2 rounded-lg 
                                                                    text-sm font-medium hover:bg-purple-600 transition-colors'
                                                            >
                                                                View {currentOffer.applicableCategories.length}{" "}
                                                                Categories
                                                            </button>
                                                        )}
                                                </motion.div>
                    ...
                    {totalOffers && totalOffers > 1 && (
                        <>
                            <button
                                ...
                                className='absolute left-0 top-1/2 -translate-y-1/2 -translate-x-4 md:-translate-x-6 bg-white
                                    text-slate-900 rounded-full grid place-items-center w-12 h-12 p-0 shadow-xl z-10 
                                    border border-dropdownBorder transform-gpu will-change-transform transition-transform 
                                    duration-200 ease-out hover:scale-110 active:scale-95'
                            >
                            ...
                            <button
                                onClick={handleNext}
                                className='absolute right-0 top-1/2 -translate-y-1/2 translate-x-4 md:translate-x-6 bg-white
                                    text-slate-900 rounded-full grid place-items-center w-12 h-12 p-0 shadow-xl z-10 border 
                                    border-dropdownBorder transform-gpu will-change-transform transition-transform 
                                    duration-200 ease-out hover:scale-110 active:scale-95'
                            >
            ...
            {openModal.type === "products" && openModal.datas.length > 0 && (
                <OfferProductsModal offer={selectedOffer} applicableProducts={openModal.datas} onClose={closeModal} />
            )}

            {openModal.type === "categories" && openModal.datas.length > 0 && (
                <OfferCategoryModal categories={openModal.datas} onClose={closeModal} />
            )}
