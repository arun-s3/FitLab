...
export default function ProductFilterForAdmin({ filter, setFilter, priceGetter, priceSetter, dateGetter, dateSetter }) {
    ...
    const statusHandler = (e) => {
        ...
        e.currentTarget.style.borderBottomLeftRadius = e.currentTarget.style.borderBottomRightRadius = showStatus
            ? "7px"
            : "0px"
    }
    ...
    const setBrands = (e) => {
        ...
        if (e.target.value.trim()) {
            ...
            if (uniqueBrands.some((brand) => !brand.trim())) {
                e.target.nextElementSibling.style.visibility = "visible"
                setBrandErrorMsg(
                    "Please make sure that there are no consecutive commas without any number between them!",
                )
                return
            }
    ...
    return (
        <div
            className='w-[20rem] border border-primary bg-white pt-[20px] pb-[25px] px-[35px] rounded-[8px] '
        ...
        >
            ...
            <div className='flex flex-col gap-[1.5rem] mt-[1rem]'>
                ...
                <div>
                    <h5 for='status' className='filter-label'>
                        {" "}
                        Status{" "}
                    </h5>
                    <div
                        id='status'
                        className='relative w-[7rem] h-[1.7rem] rounded-[7px] border
                    ...
                    >
                        ...
                        <div
                            className='absolute bottom-[-66px] flex flex-col justify-center items-center gap-[5px] w-[7rem]
                        ...
                        >
                            <span
                                ...
                                className='hover:bg-primary w-full text-center text-[12px] cursor-pointer'>
                                {" "}
                                All{" "}
                            </span>
                            <span
                ...
                <div>
                    <label for='' className='filter-label mb-[3px]'>
                        {" "}
                        Date{" "}
                    </label>
                ...
                <div>
                    <label for='brand' className='filter-label mb-[3px]'>
                        {" "}
                        Brand{" "}
                    </label>
                    <input
                        type='text'
                        id='brand'
                        className='h-[2rem] text-secondary border-secondaryLight2 rounded-[7px]'
                        onBlur={(e) => setBrands(e)}
                    />
                    <p className='text-red-500 text-[10px] tracking-[0.3px] h-[18px] w-full flex items-center invisible'>
                        {" "}
                        {brandErrorMsg}{" "}
                    </p>
                </div>
                <div className='mt-[-10px]'>
                    <h5 for='' className='filter-label'>
                        {" "}
                        Price{" "}
                    </h5>
                    <div>
                ...
                <div className='text-right'>
                    <SiteButtonSquare
                        customStyle={{ paddingBlock: "3px", paddingInline: "28px", borderRadius: "7px" }}
                        light={true}
                        lowFont={true}>
                        Save
                    </SiteButtonSquare>
