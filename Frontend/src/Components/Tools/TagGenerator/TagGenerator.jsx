    ...
    useEffect(() => {
        if (editTags) {
            ...
            setTags((prevTags) => [
                ...prevTags,
                ...uniqueTags
                    .filter((tag) => !prevTags.includes(tagConstructor(tag)))
                    .map((tag) => tagConstructor(tag)),
            ])
        }
    }, [editTags])
        ...
        if (e.target.value.trim()) {
            if (e.target.parentElement.scrollHeight > e.target.parentElement.clientHeight) {
                ...
                if (/\w+\s+/.test(e.target.value)) {
                    ...
                    setTags([...tag, tagConstructor(currentTag)])
                    e.target.value = ""
                }
            }
        }
    ...
    return (
        <>
            ...
            <div className='relative w-full h-[5rem] border border-primary rounded-[5px] bg-white tags-wrapper'>
                ...
                <input
                    type='text'
                    placeholder='Type each tag followed by a space'
                    maxlength='60'
                    id='product-tags'
                    className='w-[11rem] h-[2rem] text-[11px] text-secondary'
                    style={{ width: "10rem", height: "2rem", marginLeft: "9px", border: "0" }}
